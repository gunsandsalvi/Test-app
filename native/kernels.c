...
/* domain/company-week/income-statement.ts industrialIncome + corporateTax, verbatim order.
   daUSD is the one depreciation schedule's year-rate on the week's opening plant register
   (domain/plant.ts plantDepreciationAnnualLocal) — a seam lane the JS side reads off the register. */
...
static napi_value FrontCore(napi_env env, napi_callback_info info){
  ...
  NEXT_F64(baseGrowthRatio); NEXT_F64(baseMarginA); NEXT_F64(depreciationAnnual); NEXT_F64(openNetPpe); NEXT_F64(taxBasisOpen);
  ...
  for (int row = 0; row < n; row++){
    ...
    PnL pnl = industrialIncome(newRevenue, newEbitdaMargin, depreciationAnnual[row],
