...
export const assetManagerProfile: (input: ProfileInput) => ProfilePnl = (input) => {
  ...
  // company, its AUM IS that entity's marked book — totalAssetsLocal is recomputed weekly from
  ...
  comp.aumLocal = instEnt
    ? institutionTotalAssetsLocal(input.ctx, instEnt)
    : (comp.aumLocal ?? comp.annualRevenue * 50) * marketGrowth * (1 + flows);
  comp.managementFeeRate = comp.managementFeeRate ?? (0.005 + random() * 0.005);

  const weeklyFees = comp.aumLocal * comp.managementFeeRate / 52;
  const newRevenue = Math.max(10, weeklyFees * 52);
