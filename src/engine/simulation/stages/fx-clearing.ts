...
export function runFxClearingStage(state: GameState, ctx: WeeklyStepContext): void {
  ...
  ctx.updatedCompanies.forEach((c) => {
    ...
    const capLocal = deskNotionalCapacityLocal(leverageHeadroomLocal(sheet, bankReservesOf(ctx.v2, c.id), facilityBookOf(ctx.v2, c.id), bankBookAssetsLocal(ctx.v2, c.id)), deskStandingPfeChargeLocal(ctx, state, c.id), 'FX_FORWARD');
