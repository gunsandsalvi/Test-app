...
export interface CreditPriceWorld {
  /** The issuer's region, and — for paper no book has cleared yet — its floater quote. */
  issuerById: (id: string) => {
    region: string; leveragedLoan?: { discountMarginBps: number };
  } | undefined;
...
/** The ladder rows that are capital-markets FIXED paper — the corporate bond book's instruments. */
const isBondRow = (flags: number): boolean => !(flags & (TR_FLOATING | TR_CP | TR_FACILITY));

/** What a FIXED tranche pays and when, off its own row — the schedule its price discounts. */
export function trancheTerms(v2: V2World, row: number, week: number): PaperTerms {
  const S = v2.tranches;
  return {
    annualCouponRate: Number.isNaN(S.couponRate[row]) ? 0 : S.couponRate[row],
...
export function trancheClearedPricePerFace(
  ...
  const S = v2.tranches;
  // The market's own print, for the books that clear one.
  if (isBondRow(S.flags[row])) return clearedPriceOf(v2, instrumentId);
...
export function trancheClearedSpreadBps(
  ...
  if (!reg?.zeroRates) return undefined;
  return rowSpreadBps(v2, reg.zeroRates, row, week);
}

/** The same read for a caller that already holds the row and its region's curve — the stage-08
 *  kernels, which walk ladders by row and must not build a world adapter per firm. */
export function rowSpreadBps(
  v2: V2World, curve: ZeroCurve, row: number, week: number
): number | undefined {
  const S = v2.tranches;
  if (!isBondRow(S.flags[row])) return undefined;
  const price = clearedPriceOf(v2, v2.internedStrings[S.idRef[row]]);
  if (price === undefined || !(price > 0)) return undefined;
  const terms = trancheTerms(v2, row, week);
  if (!(terms.weeksToMaturity > 0)) return undefined;
  return spreadBpsFromPrice(terms, curve, price);
}
...
export function issuerCreditPoints(
  ...
  const reg = comp ? world.regionById(comp.region) : undefined;
  return reg?.zeroRates ? issuerCreditPointsOnCurve(v2, reg.zeroRates, issuerId, week) : [];
}

/** The same, for a caller that already holds the region's curve. */
export function issuerCreditPointsOnCurve(
  v2: V2World, curve: ZeroCurve, issuerId: string, week: number,
  /** Which of the issuer's bonds to read — a rank, say, for a caller comparing two of them. */
  rowFilter?: (flags: number) => boolean
): CreditCurvePoint[] {
  const S = v2.tranches;
  const points: CreditCurvePoint[] = [];
  for (const r of ladderRowsOf(v2, issuerId)) {
    if (!(S.principalLocal[r] > 0.01)) continue;
    if (rowFilter && !rowFilter(S.flags[r])) continue;
    const spreadBps = rowSpreadBps(v2, curve, r, week);
...
/** What this borrower pays at a maturity, for a caller that already holds the curve. */
export function issuerSpreadAtOnCurve(
  v2: V2World, curve: ZeroCurve, issuerId: string, week: number, tenorYears: number,
  rowFilter?: (flags: number) => boolean
): CreditCurveRead | undefined {
  return spreadAtTenor(issuerCreditPointsOnCurve(v2, curve, issuerId, week, rowFilter), tenorYears);
}
