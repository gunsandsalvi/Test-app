...
function p3(state: GameState, week: number): AuditFinding[] {
  ...
  const dead = state.companies.filter((c) => c.isDefaulted && (c.stockPrice > 0.01 || c.marketCap > 1e6));
  if (dead.length) out.push({ family: 'P', check: 'P3 a defaulted firm has no equity price', week, usd: sum(dead, (c) => c.marketCap), message: `${dead.length} defaulted firms still print a price or a market cap (${B(sum(dead, (c) => c.marketCap))})` });
  return out;
}
