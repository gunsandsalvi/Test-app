...
export interface Lane { from: string; to: string; ratePerTonne: number; inTransitUnits: number; shipments: number; invoicesUSD: number }
...
function lanesOf(world: World): Map<string, Lane> {
  ...
  Object.entries(world.state.freightRatePerTonneLaneMoneyByLane ?? {}).forEach(([key, rate]) => {
    ...
    out.set(key, { from, to, ratePerTonne: Number(rate) || 0, inTransitUnits: 0, shipments: 0, invoicesUSD: 0 });
  });
  ...
  (world.state.tradeInvoices ?? []).forEach((inv) => { const l = out.get(`${inv.sellerRegion}>${inv.buyerRegion}`); if (l) l.invoicesUSD += inv.amountCurrency * (inv.bookedUsdPerCurrency ?? 1); });
  return out;
}
...
export const lane = defineObject<Lane>({
  ...
  peers: {
    ...
    columns: [
      ...
      { key: 'invoices', label: 'invoiced', render: (r) => money(r.obj.invoicesUSD), value: (r) => r.obj.invoicesUSD },
    ],
  },
  overview({ obj: l, nav }) {
    return (
      <>
        ...
        <StatGrid>
          ...
          <Stat label="open invoices" value={money(l.invoicesUSD)} sub="booked on this lane" />
        </StatGrid>
