...
export const estate = defineObject<Estate>({
  ...
  peers: {
    ...
    columns: [
      ...
      { key: 'owed', label: 'claims', render: (r) => money(r.obj.claims.reduce((a, c) => a + c.principalUSD, 0)), value: (r) => r.obj.claims.reduce((a, c) => a + c.principalUSD, 0) },
  ...
  overview({ world, obj: e, nav }) {
    const owed = e.claims.reduce((a, c) => a + c.principalUSD, 0);
    ...
    return (
      <>
        ...
        <Card style={{ padding: '2px 0' }}>
          <KV k="cash" v={money(e.assets.cashUSD)} />
          <KV k="receivables" v={money(e.assets.receivablesUSD)} />
          <KV k="inventory" v={money(e.assets.inventoryUSD)} />
          <KV k="plant" v={money(e.assets.ppeUSD)} />
        </Card>
        <Table rows={[...e.claims].sort((a, b) => b.principalUSD - a.principalUSD)} keyOf={(c) => `${JSON.stringify(c.holder)}:${c.principalUSD}`} columns={[
          ...
          { key: 'owed', label: 'owed', render: (c) => money(c.principalUSD) },
