...
export const estate = defineObject<Estate>({
  ...
  peers: {
    ...
    columns: [
      ...
      { key: 'paid', label: 'paid', render: (r) => money(r.obj.distributedLocal), value: (r) => r.obj.distributedLocal },
  ...
  overview({ world, obj: e, nav }) {
    ...
    const recovered = e.claims.reduce((a, c) => a + c.recoveredLocal, 0);
    ...
    return (
      <>
        ...
        <StatGrid>
          <Stat label="assets" value={money(estateAssetsLocal(e.assets))} sub="cash · receivables · stock · plant" />
          <Stat label="claims" value={money(owed)} sub={`${e.claims.length} claimants`} />
          <Stat label="recovered" value={owed > 0 ? pctLevel(recovered / owed, 0) : '—'} sub={`${money(e.distributedLocal)} distributed`} />
        </StatGrid>
        ...
        <Table rows={[...e.claims].sort((a, b) => b.principalLocal - a.principalLocal)} keyOf={(c) => `${JSON.stringify(c.holder)}:${c.principalLocal}`} columns={[
          ...
          { key: 'got', label: 'got', render: (c) => money(c.recoveredLocal) },
        ]} />
