    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center font-sans antialiased selection:bg-blue-500 selection:text-white">
      ...
      >
        ...
        <PortfolioKpiStrip
          ...
          compositeIndices={state.compositeIndices}
          onOpenPortfolioTab={() => setState((prev) => ({ ...prev, selectedTab: 'portfolio' }))}
        />

        {/* Segmented Navigation Tab Bar */}
        <nav className="bg-slate-900/90 border-b border-slate-800/80 px-2 py-1.5 flex items-center justify-between gap-1 text-[11px] font-bold overflow-x-auto no-scrollbar">
          <button
            id="nav-tab-macro"
            onClick={() => setState((prev) => ({ ...prev, selectedTab: 'macro' }))}
            className={`flex-1 py-1 px-1 rounded-lg transition-all flex items-center justify-center gap-1 ${
          ...
          <button
            ...
            onClick={() => setState((prev) => ({ ...prev, selectedTab: 'portfolio' }))}
            className={`flex-1 py-1 px-1 rounded-lg transition-all flex items-center justify-center gap-1 ${
              state.selectedTab === 'portfolio'
                ? 'bg-blue-600 text-white shadow-sm'
                : 'text-slate-400 hover:text-slate-200'
            }`}
          >
