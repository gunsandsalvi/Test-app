...
export const RatesScreen: React.FC<{ state: GameState, onOpenTrade: (i: any) => void }> = ({ state, onOpenTrade }) => {
  ...
  const govBonds = (reg.govDebtTranches || []).map((t: GovDebtTranche) => {
    ...
    const livePrice = priceCorporateBond(remainingTenorYears, t.couponRate, reg.yieldCurveParams, 0, false, 0).price;
    return {
      id: t.id,
      name: `${selectedRegion} ${remainingTenorYears.toFixed(1)}Y Gov Bond`,
      ticker: t.id,
  ...
  return (
    <div className="p-3 space-y-4 pb-20">
      ...
      <div className="p-4 bg-[var(--bg-elevated)] rounded-xl border border-[var(--border-hairline)] space-y-4">
         ...
         <div className="relative mx-auto" style={{ width, height }}>
            <svg width={width} height={height} className="overflow-visible">
               ...
               {govBonds.map(b => {
                  ...
                  const cx = ((lowerI + frac) / (tenors.length - 1)) * width;
         ...
         <div className="pt-6 flex justify-between text-xs border-t border-[var(--border-hairline)] text-[var(--text-secondary)]">
