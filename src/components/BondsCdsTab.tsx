...
export const BondsCdsTab: React.FC<BondsCdsTabProps> = ({
  ...
  const [viewMode, setViewMode] = useState<'CDS' | 'CORP_BONDS' | 'LEVERAGED_LOANS'>('CDS');
  ...
  return (
    <div className="space-y-3 pb-20">
      ...
      <div className="grid grid-cols-3 gap-1 p-1 bg-slate-900 rounded-xl border border-slate-800 text-[11px] font-bold text-center">
        ...
        <button
          onClick={() => setViewMode('CORP_BONDS')}
          className={`py-1.5 rounded-lg transition-all flex items-center justify-center gap-1 ${
            viewMode === 'CORP_BONDS'
            ...
            >
                  ...
                  {viewMode === 'LEVERAGED_LOANS' && (
                    <>
                      <div className="text-sm font-extrabold font-mono text-amber-300">
                        {comp.isDefaulted ? '65.0% Par' : `${comp.leveragedLoan.pricePar.toFixed(2)}%`}
                      </div>
              ...
              <div className="mt-2.5 pt-2 border-t border-slate-800/80 flex items-center justify-between text-[10px]">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {viewMode === 'LEVERAGED_LOANS' && (
                    <button
                      id={`btn-trade-loan-${comp.ticker}`}
                      onClick={() =>
                        onOpenTrade({
                          assetType: 'LEVERAGED_LOAN',
                          id: `${comp.id}_LOAN`,
                          symbol: comp.ticker,
                          name: `${comp.name} Senior 1st Lien Loan`,
                          region: comp.region,
                          price: comp.leveragedLoan.pricePar,
                          quoteUnit: 'pts of par',
                          details: {
                            sector: comp.sector,
                            rating: comp.creditRating,
                            tenorYears: comp.leveragedLoan.tenorYears,
                            quotedMarginBps: comp.leveragedLoan.quotedMarginBps,
                            discountMarginBps: comp.leveragedLoan.discountMarginBps,
                          },
                        })
                      }
                      disabled={comp.isDefaulted}
                      className="px-2.5 py-1 rounded bg-amber-600 hover:bg-amber-500 active:scale-95 text-white text-[10px] font-bold shadow-sm transition-all disabled:opacity-40"
                    >
                      Trade Loan
                    </button>
                  )}
