...
export interface GameState {
  ...
  /** §3.21 — the books that did not clear last week, and why; their prints were carried. */
  lastWeekUnclearedBooks?: string[];
  ...
  lastWires?: { count: number; byKind: Partial<Record<string, number>>; valueUSDByKind: Partial<Record<string, number>>; /** money wires recorded after the last pass — they settle next week (N: dated wires) */ moneyPendingLocal: number; /** §3.13c: the week's money wires per currency, in that currency's units */ moneyByCurrency?: Record<string, number>; /** §3.13c: the dated tail per currency */ moneyPendingByCurrency?: Record<string, number>; /** §5-WIRES W2: the clearing house's net per `region|kind` after the week's wires */ houseNetUSDByKey?: Record<string, number>; houseNetUSDByAsset?: Record<string, number>; /** §5-WIRES W3: the issuers' net per `region|kind` */ issuerNetUSDByKey?: Record<string, number>; issuerNetUSDByTicker?: Record<string, number>; /** W5: the register's net per asset kind, in shares */ registerNetQtyByKind?: Record<string, number>; registerNetQtyByHolder?: Record<string, number>; /** §5-WIRES W4 */ goodsNetUnitsByKey?: Record<string, number>; goodsFlowByKey?: Partial<Record<string, { producedUnits: number; consumedUnits: number; scrappedUnits: number }>>; goodsOutUnitsByKey?: Record<string, number>; goodsInUnitsByKey?: Record<string, number>; goodsDeliveredByKey?: Record<string, number>; goodsInByTicker?: Record<string, number>; /** §3.26-f-iii W6: plant wires per firm in units of cost, and the week's transformations */ plantNetCostByCompany?: Record<string, number>; queueNetCostByCompany?: Record<string, number>; plantFlowByCompany?: Partial<Record<string, import('./plant').PlantFlow>>; /** §3.26b-i W7 */ dwellingNetUnitsByRegion?: Record<string, number> };
