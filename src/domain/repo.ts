 ...
 * A position was a scalar `repoLentUSD` beside books that itemize everything else (rule 3); who
 * lent to whom was unknowable, so the cash leg had no counterparty and the collateral leg had no
 * owner (rule 14); and the pledge was one number, so no particular bond was encumbered and
 ...
 * book, with both parties named — which is what makes it two-sided (rule 14) without being two
 * copies of one thing (rule 3). Every scalar the sheets used to carry is derived from it.
 *
...
/** The specific paper pledged: which bucket, and how much face of it. */
...
export interface RepoContract {
  ...
  principalUSD: number;
  /** Annualised decimal, struck at this session's cleared level (rule 9). */
  rateAnnual: number;
  struckWeek: number;
 ...
 * bank has pledged, by bucket, at face: 07c and 07f read it as a floor on the buckets it actually
 * touches, so pledging thirty-year paper stops constraining the two-year book.
 */
export function encumberedFaceByBucket(book: RepoContract[], ticker: string): Map<string, number> {
  const byBucket = new Map<string, number>();
  book.forEach((c) => {
    if (c.borrowerTicker !== ticker) return;
    c.collateral.forEach((p) => byBucket.set(p.bucketKey, (byBucket.get(p.bucketKey) ?? 0) + p.faceUSD));
  });
  return byBucket;
}

/** Total face pledged by one bank across every bucket. */
export function encumberedFaceUSD(book: RepoContract[], ticker: string): number {
  let faceUSD = 0;
  encumberedFaceByBucket(book, ticker).forEach((v) => { faceUSD += v; });
  return faceUSD;
}
