...
`plantVintagesOf`; since g-ii-d the rows are the only register of plant there is — every writer
reads them, applies the vintage function and writes them back, every reader reads them, and
`Company.plant` is gone.
