...
| **B5 FORBID the mechanism does not add demand to make itself clear** | `src/engine/simulation/stages/financial-clearing-engine.ts:solveClearingStat` | ✅ |
...
`maxNetPurchaseLocal: orderLocal`, where
`orderLocal` is `plannedPurchasesByBond` (`central-bank-demand.ts:41,48`) — a policy quantity struck
before the book opens and unable to respond to weak demand; an index fund the same with
`maxNetPurchaseLocal: availableCashLocal` (`etf-demand.ts:37-51`). Neither size is a residual.
...
its `leftoverLocal` defect (`:110`) is the standing guard that it stays deleted.

**The desks are bounded participants, not a backstop.** `dealerDeskCapacityLocal`
...
(`unsoldStaysWithHolder` off): `dealerInventory = liveFloatLocal − allocatedLocal` (`:889`). On repo,
...
(`dealerLongBaseByPair = −sign × |residualLocal|`) with no capacity test on the way in, so the
...
### ⚠️ D1.a — THE ENGINE'S PRIMITIVE IS A PRICE NOW, AND THE FIELD THAT SAYS SO IS STILL PER ADAPTER

Every book runs `statKind: 'PRICE_LIKE'` since §9.13-SOV row 4 and §9.13-CREDIT rows 1, 3 and 4:
what the auction solves for and prints is a price, and yield, spread and DM are read back off it.
What is left is that `statKind` is hard-coded beside each adapter while `assets/index.ts` declares
`quotedAs` separately — two representations of one fact, which is **§3 step 13-BOOK (d)**: the
registry becomes what the adapter reads, and the field goes.
