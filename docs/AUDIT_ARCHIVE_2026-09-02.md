# THE AUDIT ARCHIVE — the 2026-09-02 sweep, as the reviewers wrote it

Moved out of `MASTER_PLAN.md` on 2026-09-03. It was §8 there: 1,771 lines, 47% of the file, and
the plan's own header calls it a RECORD rather than a work list. §3 of the plan cites none of its
rows by name, so nothing in the live work list depends on where it lives.

**READ THE WARNING IT CAME WITH.** These findings are as the reviewers wrote them and were NOT
re-verified. At least one was measured WRONG on a check (`MAX_WEEKLY_FX_MOVE_PCT` was reported as
a live damper and is in fact a dead export). Treat every unconfirmed row as a lead with a
file:line, not a fact — and note that the line numbers are from 2026-09-02 and the tree has moved
a great deal since.

`MASTER_PLAN.md` §3 remains the only work list. A row here that is not yet a step becomes one at
its logical place when it is picked up.
