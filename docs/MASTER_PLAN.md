...
**THE BASELINE IS STRUCK: §7.244** — 628 violations in 94 families at 60 weeks, on the world as
of §7.243's seed fix (the pure intermediate that priced at $0, found by §7.242's guard on its
first run). Every comparison of a live number is against §7.244's decomposition and nothing
earlier.
...
intermediate's seed price). **THE BASELINE IS NOW STRUCK: §7.244 (628 / 94, 60 weeks) is the
one valid comparison run.** Items 1, 2, 4 and 7 below carry §7.244's first readings (fill 0.00x,
unemployment ~34% in all four regions, damper 2,189, CPI ×89.7); the finer reads are still owed
against it. The kernel
...
| **THE LABOUR MARKET FAILS OVER A LONG HORIZON** | **RE-DIAGNOSED IN §7.224: nothing in the labour market is wrong.** It reads `nominalGrowth - inflation`, and the inflation was a goods-market defect at week 1. With that fixed, EUR's 66% is gone and all four regions sit near 33%, but JPN now reaches 75% by week 58 and 69 band violations remain. §7.233's run added EUR back at 84.2%; **§7.244 (the valid baseline) removes that outlier: all four regions converge on ~34% by week 60** — the labour market prints the goods famine (fill 0.012) uniformly now. One new defect signature in its own series: **the vacancy rate prints 2.0e9%** — an unguarded ratio at a limit, §7.210's exact shape. **Do not touch the matching function or the quit rate until the capacity row below is closed** — but the vacancy overflow is a units/guard defect, findable by measuring the week the v series first departs sanity. Owner: HH5/LAB, after IND. |
| **SUPPLIER MARKET SHARES DO NOT SUM TO 100% — REOPENED AS THE EUR REGRESSION** | Was largely closed by §7.224 (down to one category). **§7.233 reopened it: the SME capacity fix — the correct rule (§1.20: keep it) — destabilised EUR alone.** §7.244 re-measures it on the valid baseline: **~124 violations, all but four in EUR, across 16 EUR categories** (`EUR:industrial_chemicals` 20 of them — §7.243 gave that good a market and the share defect was waiting inside), and **EUR's character changed: no longer a labour outlier (30.1% against 27–36% everywhere) but a NOMINAL one** — household income 62,116B against USA's 1,832B, outlays 4,573B, TGA −5,164B, imports 2,628B against 3.5B exports, and it leads the fiscal, ownership-conservation and bypass families too. **§7.245 TOOK THE TRACE: the fiscal half was an unappropriated capex-category government bid (fixed — the PUB1e-deleted fallback is gone), and the nominal runaway is the four IN_PLACE services (housing rental ×48, repair ×40, electricity ×26, professional ×23 over six weeks) — the §7.132 floored-at-cost ratchet crossing gain 1 where service slack is least: named firms' costs chase their own price, §7.139's shutdown kills their offers, and inelastic unit demand clears at the §7.208 ladder's top. USA had the same event and recovered; EUR's floors outran its price.** Next action: the mechanism, not a patch — a service's floor anchors to the WAGE its staff are paid, not intensity × its own revenue (§7.133's lesson, avoiding §7.133's failed per-head form). The share-sum violations (w38+) are downstream of the collapse. Owner: IND/COH, with the price-level row. |
| **REPO COLLATERAL IS OVER-PLEDGED** | The largest single family in §7.211 (XIVF 56x, THSY 54x): a bank pledging more of a tenor bucket than it holds, while `reconcileRepoPledges` is supposed to reconcile weekly. **§7.230 found the check itself split: the engine reconciled at $1, the harness checked at 1e6.** Both now read `domain/collateral.ts`'s single 1-dollar tolerance, and the count went **97 → 281 (§7.233) — visibility, not degradation: every one was there before and could not be seen.** §7.244 (the valid baseline): **129, concentrated in two banks — XIVF 73, ADFI 45, WMQC 10 — still the largest single family.** Judge future runs against §7.244. Owner: WS6/REPO. |
| **GOVERNMENT OUTLAYS EXCEED THE BUDGET** | **The check that watched this row was wrong (§7.230): it compared outlays against `governmentSpendingUSD × 1.5`, which is not the budget**, and fired seven times on the wrong quantity. It now reads `Government`'s own decomposition (contractual interest and payroll off the top, discretionary remainder scaled by the stance) and fires naming whether the overrun is contractual or discretionary. §7.244 (the valid baseline): **58 — EUR 49, JPN 5, USA 3, UK 1 — EUR-led like every nominal family in that run**, and EUR's TGA ends at −5,164B. Next action: read the EUR 49 WITH the EUR-regression row above — a contractual overrun (interest paid on real dates) is the stance rule's problem, which is MAC's fiscal step function, not a ledger defect; an overrun that is EUR's hyperinflation wearing a fiscal costume belongs to that row. Owner: MAC, behind the EUR row. |
| **THE PRICE LEVEL DOES NOT SETTLE** | **PARTLY CLOSED BY §7.224, AND THE OLD DIAGNOSIS WAS WRONG.** This row blamed COH4's demand ladder. The ladder is not the cause: two unit errors were, both at week 1 — the seasonal factor inside the production shutdown test (whole categories shipping zero) and a CPI whose base and current prices were different price concepts. Both fixed; mid-run inflation 2,266% -> 145%, week-60 50.8%, and the hyperinflating revenue prints (6.6e+21) are gone. §7.244 (the valid baseline): **CPI ×89.7 over 60 weeks, goods fill decaying to 0.012, active firms 2,496 → 1,974 — the ratchet is steeper on the re-based world** (not comparable to §7.233's ×15.4 in level, same shape). Its cause is unchanged: capacity dispersed against demand, short markets clearing at the reservation and long ones floored at cost — and §7.244 shows the accounting spine surviving it (claims, trade, receivables all exact), so **the price level is the work, not the books.** **Still do not put the bid premium back and do not cap the bid.** Owner: IND, via the capacity row. |
| **THE CAPITAL-GOODS SECTOR CANNOT SUPPLY THE ECONOMY'S CAPEX** | Firms bid their real capex and the basket weights sum to 1, so the bids ARE the capex figure. §7.178 made the demand LEVEL honest and §7.199 closed the seed's investment fixed point; **§7.244 collected the first reading on the valid baseline and it is TOTAL famine: supplied/demanded 0.00x in ALL FIVE categories, bids 8.02x what the industries were built for, prices 10–50x base** — worse than §7.168's 8% fill, though the ×90 price level inflates the dollar figures. It is the accumulated cost behind the ~34% unemployment all four regions now converge on (§7.179's mechanism, uniformly). |
| **THE BOOKS PRINT THEIR DAMPERS** | The engine states its own failure condition: a name clamped for weeks means the posted schedules disagree with the printed level and **the print is the damper, not the market.** §7.244 (the valid baseline): **2,189 persistently bound, worst streak the full 60 weeks** (the old 947 benchmark is void with the re-base). §7.197 records the rise as the honest shape of deleting the residual dealer: **the pressure was always there.** **Do not widen the damper.** The well-posed question is: who buys when the holders as a group want less than they hold? Owners: SCALE (the float half), and the measurement run. |
...
| **THE UNMODELED BOUNDARY — FOUR declared frontiers** | `BOUNDARY_FRONTIERS` in the harness declares every reason that may still cross with its owner; anything else fails on the week it appears. **It is a list to shorten, never a place to add a line to make a run pass.** §7.240 found a FIFTH entry living only in the harness (`freight on a lane no carrier serves`, harness.ts:163, owner comment "the freight book") that this row never blessed — either record it here as the fifth with its shrink condition (the fleet reaching every lane) or close it; a frontier the plan does not know about is exactly the quiet-add this row forbids. Standing: `non-auction operating receipts` (now the accrual lag between a week of sales and annual revenue — §7.207 changed what it measures), `other opex beyond auction settlements`, `inventory carrying cost` (only a region with no distribution firm — §7.205), and `estate distribution`. **Closing the first entirely is a PAIR with the cost line beside it:** both are accruals, and removing one side's boundary settlement without the other makes every firm bleed cash. |
