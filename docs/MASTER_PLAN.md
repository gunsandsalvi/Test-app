...
| **THE UNMODELED BOUNDARY — six declared frontiers left, and it BREAKS now (§7.197)** | `{ kind: 'UNMODELED' }` absorbs whatever is paid to it and pays whatever is asked of it; from the inside that is indistinguishable from a leak, and until 2026-08-30 nothing checked it — 25 reasons were crossing it. It is now a harness violation: `BOUNDARY_FRONTIERS` in `scripts/harness.ts` declares every reason that may still cross with the project that owns it, and **anything else fails on the week it appears**. It is a list to SHORTEN; adding a line to make a run pass is the defect this check exists to catch. What is left, 10 weeks: **non-auction operating receipts 6.7B / non-auction opex -2.3B** — the goods market's own edge, buyers and suppliers outside the registry (owner: SCALE/PROD, the completeness work); **commercial paper 0.6B** — issued at a derived rate with no cleared book, so its holder has no name and the money funds are its real buyer (owner: the CP book); **inventory carrying cost** — warehousing, a service nobody sells (owner: SVC); **freight on goods purchase** — the carriers EXIST and earn it as revenue, what is missing is the buyer→carrier leg by lane share (owner: the freight book); **estate distribution** — a workout's residual claimants (owner: G5). |
