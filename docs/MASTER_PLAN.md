...
18. **Model updates come first; a misbehaving NUMBER is not a work item.** (User directive,
    ...
    - **AND THE OTHER HALF, learned the hard way (§7.179).** When a number is so far out that it
      BLOCKS unrelated mechanism work, closing the gap IS mechanism work. Measured: two correct
      changes in a row could not land because each removed a cushion and pushed unemployment over
      its band — the economy runs at ~29%, so anything honest crosses 30%. **The answer is never
      to widen the band (rule 2) or to stop making correct changes.** It is to find which MISSING
      MECHANISM the number is the accumulated cost of, and build that. Here it is the supply side,
      which is why it is now item 2. **Rule 18 forbids scheduling around a number; it does not
      forbid noticing that a number is a symptom with an owner.**
...
| 2 | foundation | **SUPPLY/CHAIN — the economy cannot make what it is asked for** *(promoted from item 20, 2026-08-30)* | **THE UNBLOCKING ROW (§7.179).** Two consecutive CORRECT changes could not land because each removed a cushion and pushed unemployment over its band (§7.177, §7.179) — the economy runs at ~29% and the band has become the binding constraint on what can be built. **That 29% is not a labour defect** (§7.149 fixed the one that existed); it is the accumulated cost of the SUPPLY side. **THE FIXED POINT IS CLOSED (§7.199):** firm size and investment demand are solved against each other rather than each taking a different number, and the producer base is re-dealt against the authoritative vector. CAP's capacity-off-PP&E is restored (it landed with §7.180). **What remains is the ORDER GRAPH** — recipes are deep since CHAIN-D but DEMAND propagates upstream, not ORDERS, so there are no lead times, no per-tier inventories and therefore no bullwhip; `CONTRACTED_DEMAND_SHARE = 0.6` dies with it. §5 calls that ordinary depth-tier work. **Unmeasured.** CHAIN-D and CHAIN-E are done (§7.117-118, §7.120). Prereqs BP1 and IND10/11, both now done. |
| 3 | foundation | **CAP — a firm can run a loss; then production and capacity** *(clamps)* | **THE CLAMP PROGRAMME IS DONE (§7.176); the capacity DECISIONS remain.** Deleted this pass: the production-response [0.5,2.0] band, the input-supply-constraint 0.3 floor, the SME pool's half-the-reference offer floor, the growth-capex 0.4 floor (whose own comment had already convicted it), and the ±2%/week capacity clamp (§7.167). `CAPEX_PUBLIC_SUPPLY_SHARE` was dead — imported, never read — and is gone with them. The carrier's offer floor now includes the capital charge on the hulls, so a balanced freight market no longer clears where no ship can be replaced. Maintenance capex is anchored to depreciation (§7.167). **BOTH REMAINING ITEMS ARE NOW DONE, and one of them was already done when this row still said otherwise.** Capacity read off PP&E landed with §7.180's commit — the row's "TRIED AND REVERTED (§7.177)" was stale, which is why §7.199 says to read the code before believing a row. The capex SUPPLY row closed at the seed: §7.199 solves firm size and investment demand against each other instead of using a different number on each side. **Unmeasured — the capital-goods fill rate against §7.168's 8% is the first thing the next run must read.** |
| 4 | foundation | **COH — cohorts accumulate: household balance sheets** | **COH1, COH2 AND HALF OF COH3 DONE 2026-08-30 (§7.199).** Spendable and accumulated are two stocks, so the buffer rule has something to be a buffer of and dissaving drains the buffer before it sells anything; the pension contribution is the life-cycle saving the cohorts already decide on (`PENSION_CONTRIBUTION_RATE` deleted); a pension fund opens as big as the entitlements it owes, derived from the age structure; the tautological income assert is gone. **COH4's SAVINGS-DESTINATION HALF CLOSED 2026-08-30 (§7.204):** `HOUSEHOLD_SAVINGS_TO_DEPOSITS_SHARE = 0.3` is the seed only — where the week's saving goes is the motive split COH2 already makes, measured and published as `householdState.liquidSavingShare`, and both readers take the same number. **REMAINS: (a) the WAGE LEVEL, which `LABOR_SHARE_OF_OUTPUT = 0.62` still sets through `getBaseAnnualWageUSD` — the derivation is written in §6.1 and it is the highest-risk unmeasured change available, so it wants a run either side of it; (b) COH4's BID PREMIUM, whose two candidate derivations differ by 100x and are both in §6.1 with the reason neither can be picked blind; (c) the last slice of `INSTITUTIONAL_OPENING_BOOK_SHARE`, `equity: 0.42`, which needs an asset manager anchored on the households' own fund holdings.** |
| 5 | foundation | **DEM — demographic variability** | **CLOSED (§7.181) — this row was stale.** People age on a Gompertz hazard, the four life-cycle shares are bands of the seed's own stationary age structure, and the life-cycle savings rate falls out of it. COH2 consumed it (§7.199): the pension contribution and the entitlement stock are both derived from the age structure now. *Original text follows.* **THE AGE STRUCTURE ALREADY EXISTS AND NOTHING READS IT (§7.169).** `lifeCycleDistribution` carries four stages with `shareOfPopulation`, `savingsRate` and `consumptionMultiplier` per stage; only `RETIRED.shareOfPopulation` is ever read, and only to set a death rate. **`LifeCycleStageData.savingsRate` is read NOWHERE** — the life-cycle saving motive is present as data and binds on nothing, the fifth instance of that pattern this session. **And it should not simply be wired in: those per-stage rates are stated shape parameters** (§5-DIST-P), and the derivation is better than they are — see the row's §5 entry. | Small; takes the population-growth and migration clamps. **Promoted in substance: its AGE STRUCTURE half is what gives the household sector a life-cycle**, and a life-cycle is what makes a positive steady-state savings rate an outcome rather than a coefficient (§5-DIST-P). |
| 6 | markets | **HSG — a housing market that clears** *(clamps)* | **CLOSED 2026-08-30 (§7.159-160, §7.200) — UNMEASURED.** The price clears (marginal-buyer affordability walked down the tiers, floored at the build cost), the mortgage book is vintages marked to today's prices with a 5-year fix and reset, borrowing capacity is `DSTI x income / annuity factor`, every bank quotes its own mortgage off its own loss experience and the borrower shops, and turnover is one sale per tenure plus the owners who can now afford to trade up. Every constant the row named is gone: the [0.5, 3.0] price bound, the credit factor, the `\|\| 400000` fallback, the drift, `MORTGAGE_SPREAD_OVER_10Y_BPS` and `HOUSING_TURNOVER_RATE_ANNUAL`. **What it owes a run: §5-HSG's three verify conditions, which had nothing to read before today.** *Original text follows.* **PROMOTED from item 10; substantially started 2026-08-30.** Done: the mortgage book is a cross-section of **vintages** each marked to today's prices, so severity is `E[f(LTV)]` and the model can have a mortgage credit event — a −20% price move now takes severity to 2.1x where the one-average book said its floor (§7.159); loans carry a **5-year fix and RESET** (17.5% of the book reprices a year) with per-vintage default frequency; and borrowing capacity is `DSTI x income / annuity factor` (§7.160). **What remains is the row's actual title: `medianHomePriceUSD` does not CLEAR.** Until it does, the affordability limit has slack it should not always have, and `HOUSING_TURNOVER_RATE_ANNUAL` is still a constant on the seller's side. |
| 7 | markets | **CRD — credit prices cleared, ratings handle zero earnings** *(clamps)* | **CLOSED 2026-08-30 (§7.162, §7.184, §7.201) — UNMEASURED.** The CDS half is built: single-name protection clears against real hedging demand (the large-exposure limit on a bank's own book, which is `09-concentration-risk`'s first consumer) with the desks and credit funds writing it, and the basis against the cash OAS is published per issuer. The ratio clamps are gone, answered in the rater — a firm with no earnings rates off liquidity. **THE CRD x COH JOIN IS DONE (§7.205) — the row is fully closed, UNMEASURED.** Credit tiers and wealth tiers are two partitions of one population, so they go on one axis rather than being mapped to each other: the BUFFER, months of its own spending a household could cover from what it holds liquid. Each credit tier inherits the balance sheets in its band of that ranking, which retires four stated delinquency multipliers and a debt split by head count. *Original text follows.* **Household half DONE (§7.162); RATINGS half DONE (§7.184); the CDS half is now UNBLOCKED (G3 closed).** Tier migration is two-way on measured delinquency, delinquency heals, the tier rate is quoted from that tier's own measured loss; and a rating is no longer two ratios — scale, customer and supplier concentration, the maturity wall against liquidity, and earnings volatility each notch it, every one a measurement the model already took. **Remains: the CDS half, which needs G3.** |
| 8 | foundation | **IND — industry operating models** | **CLOSED (§7.146-156, §7.205) — UNMEASURED.** IND16 landed as the refactor its own scoping said it had to be: the household's `facilities_and_logistics` buyer-mix share is GONE and its distribution spend arrives as a channel margin on the goods it is actually spent distributing, so the sector is paid once. The margin is the good's own physics — the cover the channel holds (the lead time it bridges, capped by shelf life) at what holding it costs (carrying cost plus the short rate) — so a service has no channel and nobody had to say so. Three price levels now, each with a real payee, and the CPI reads the shelf one. **It also closed a declared boundary frontier: warehousing has a seller.** |
...
| 12 | markets | **HF — hedge fund strategies + prime brokerage** | **CLOSED 2026-08-30 (§7.190).** Four funds instead of one, each in the markets its strategy is actually in; leverage is a real prime-brokerage line from a named bank, at a derived haircut, consuming the broker's balance sheet, priced, and withdrawable — so a margin call is a mechanism. Three of the four folded constants are gone: the FX elastic side is the pair's own volatility against the fund's own capital, the hedge ratio is a mandate property, the LBO debt share is 07d's answer. **The fourth, `HOME_BIAS_BY_ENTITY_TYPE` used as a weight, was CHECKED and the review's claim does not hold** — see §7.190. **THE EQUITY SHORT IS BUILT (§7.202) — UNMEASURED.** A securities-lending market clears the borrow FEE on the same engine as every other book: the float is the borrow demand, the participants are the holders who will lend, an unfilled borrow is a locate that failed, a lender selling out from under its loan is a recall, and the recalled borrower's buy-in is a mandated purchase with no reservation price — which is a squeeze. `stockLoanNetUSD` carries the position on both balance sheets and `shortInterestShares` measures the book. No new free parameter: the lender's reservation fee is the capital its one-week gap consumes at its own required return, and the short's size is the mirror of the long schedule the same fund already runs. **What it owes a run:** whether any borrow clears at all, the fee distribution across names, and whether a recall cascade squeezes or oscillates. |
| 13 | markets | **DER — derivatives and the people who hedge with them** | **TWO SLICES DONE 2026-08-30 (§7.191): IRS and the cross-currency basis.** The swap market clears par rates at 2/5/10Y off exposures the model already measures, and prints the SWAP SPREAD — the first cross-market basis, and the first test that two of its markets agree. The basis is a cleared price: hedger demand against what the desks can write, with the walk-away tolerance derived from the currency's own volatility and the mandate's own hedge ratio. **ALL FIVE SLICES ARE NOW BUILT — UNMEASURED.** CDS landed with CRD (§7.201), the option vol with it, and §7.202-203 closed the last two: the commodity curve clears against real producer and consumer hedging with the storage desks holding its top and the convenience yield inferred from it, and the corporates are in the FX book against the same desks and the same basis. **The row's remaining debt is entirely a MEASUREMENT one — §5-DER's verify list: the swap spread and CDS-cash basis holding single-digit bands in calm weeks and blowing out under stress, contango when inventories are high and backwardation when scarce, expiry convergence within the dealer spread, and hedged firms demonstrably feeling less P&L from the shocks they hedged.** |
...
| **OWN7** — the corporate over-holding | §7.104, §7.197 | Harness 602 → 107. **A float is what the participants in THIS book can hold between them** — not the whole issue, because a holder that does not bid keeps its position. **Finished 2026-08-30 (§7.197):** the rule reaches all five stock books, the seed places exactly the stock those books clear (it was placing 132% of the corporate stock and leaving 20% of the sovereign stock with nobody), and **the residual dealer is deleted** — an unsold holding stays with its holder, so the float rule no longer needs a counterparty outside the participant set to make the arithmetic close. |
...
### SUPPLY/CHAIN — The economy cannot make what it is asked for  *(item 2 — promoted 2026-08-30)*

**THIS IS THE UNBLOCKING ROW.** §7.179: two consecutive CORRECT changes could not land because
each removed a cushion and pushed unemployment past its band. The economy runs at ~29%, so the
band now decides what can be built — and the 29% is the accumulated cost of THIS row.

**What it owns, in the order the measurements point:**
1. **The capital-goods sector fills ~8% of the capex bid at it** (§7.168). Four of five categories
   short, prices 65-174% over base — the price mechanism working and supply not.
2. **Its capacity was seeded off a frozen number** (§7.178 fixed the weekly level, 1.55x → 1.29x;
   the SEED half remains and carries a genuine fixed point, since firms are generated FROM
   category demand).
3. **Recipes are one tier deep** against real chains that are graphs — the original CHAIN scope.
4. **CAP's capacity-off-PP&E restores the moment this closes** (§7.177, written down in full).

CHAIN-D and CHAIN-E are done (§7.117-118, §7.120). Prereqs BP1 and IND10/11, both now done.
...
| **THE BOOKS PRINT THEIR DAMPERS — promoted from §6.2, 2026-08-29; G3's half landed 2026-08-30, UNMEASURED** | The clearing engine states its own failure condition: the damper "must never BIND persistently — a name clamped for weeks on end means the posted schedules disagree with the printed level and the print is the damper, not the market." The watchlist row set the same test ("only wrong if it stays there") at 1,961 bound; the harness now prints **2,549 persistently bound, worst streak 60 weeks in a 60-week run**. The condition is met. This is ONE defect wearing many prints: the FX rate pinning at −8.01% (its damper to the second decimal) 38–39 weeks in 40, and the five books' `MAX_WEEKLY_*_MOVE_PCT` binding across 2,549 instruments — the posted demand does not reach the float it is asked to clear (§7.18's want/have from the demand side). The engine is not the defect; the thin side is. **Owners: G3 (dealer capacity, G3d — DONE §7.185, count not yet re-measured), SCALE (float), XB6 (the FX flow), HF (the FX elastic side).** Do not widen any damper. **The next measurement run must print the persistently-bound count against 2,549/60 weeks: that is the test of whether real dealer capital was the missing absorber.** **UPDATED 2026-08-30 (§7.197, §7.198): the 10-week probe count went 890 → 947 across the OWN7/CASH/CP sweep, and that is a REGRESSION worth having.** It peaked at 1042 before the desk-position fix in §7.198, which is how that bug was found — a count that moves the wrong way is a diagnostic, and this one paid for itself. The float is now what holders actually hold, the treasury's supply is real and one-way, and neither is absorbed by a dealer nobody named — the residual dealer that used to take the other side was hiding exactly this pressure. The question the owners face is now well posed: **who buys when the holders as a group want less than they hold?** SCALE's answer is a bigger holder base; G3's is more desk capital; the third answer, which nothing has yet, is a holder whose reservation is low enough to bid at the wides. Still: do not widen any damper. |
...
| **THE UNMODELED BOUNDARY — FOUR declared frontiers left, and it BREAKS now (§7.197, §7.198)** | `{ kind: 'UNMODELED' }` absorbs whatever is paid to it and pays whatever is asked of it; from the inside that is indistinguishable from a leak, and until 2026-08-30 nothing checked it — 25 reasons were crossing it. It is now a harness violation: `BOUNDARY_FRONTIERS` in `scripts/harness.ts` declares every reason that may still cross with the project that owns it, and **anything else fails on the week it appears**. It is a list to SHORTEN; adding a line to make a run pass is the defect this check exists to catch. What is left, 10 weeks: **non-auction operating receipts 6.1B / non-auction opex -2.4B** — the goods market's own edge, buyers and suppliers outside the registry (owner: SCALE/PROD, the completeness work); **inventory carrying cost** — warehousing, a service nobody sells (owner: SVC); **estate distribution** — a workout's residual claimants (owner: G5); and `freight on a lane no carrier serves`, which measured ZERO. Closed since: commercial paper (its own cleared book, §7.198) and the carriers' freight leg (paid to the carrier by name, §7.198). |
| **THE WAGE LEVEL IS A STATED LABOUR SHARE — COH3's remainder, with its derivation (§7.199)** | `getBaseAnnualWageUSD` sets every occupation's base wage as `productivity x LABOR_SHARE_OF_OUTPUT`, with the share stated at 0.62. It is read EVERY WEEK by eight call sites, not just at the seed, so it is the anchor under household income, the labour market, every employer's payroll and the freight market's crew cost. **The derivation that replaces it:** a firm's value added splits into labour compensation and gross operating surplus, and the surplus must cover depreciation plus the return the capital's owners require — so `labourShare = 1 − (depreciation + cost of capital) x (capital / value added)`, with the capital-output ratio from `SECTOR_PPE_INTENSITY` over `(1 − recipe intensity)` and the cost of capital from the policy rate plus `EQUITY_RISK_PREMIUM`. Every input already exists. Sanity-checked by hand it lands in 0.64-0.88 across plausible capital intensities, which brackets the 0.62 it replaces. **NOT LANDED DELIBERATELY: it changes the wage LEVEL, which moves household income and therefore everything, and it needs `getBaseAnnualWageUSD` to take a cost of capital it currently cannot see (8 call sites).** This is the one change in the foundation tier that genuinely wants a measurement either side of it. Owner: COH3. |
| **THE HOUSEHOLD BID PREMIUM — COH4's remainder, with BOTH candidate derivations and why neither can be picked blind** | `HOUSEHOLD_BID_BASE_PREMIUM = tanh(0.05) x 0.15` (a frozen constant wearing arithmetic) times `HOUSEHOLD_BID_PREMIUM_BY_TIER = {2.5, 1.0, 0.35}` (chosen elasticities) sets what a household will pay ABOVE the going price in stage 05 — every consumer category, every week. The tier itself is a real registry property (rule 17) and stays; the numbers are the defect. **Two derivations, both from measured cohort quantities, and they differ by two orders of magnitude.** (a) *The budget-redirection bound:* a household defending a staple can move its whole discretionary pot onto that line, so the reservation is where staple spend equals the entire budget — `premium = (1 − stapleSpendShare) / stapleSpendShare`, ~1.9 at a 0.35 staple share. True of the FIRST units and budget-consistent by construction. (b) *The marginal reservation:* the bid is one step for the whole quantity, so its max price is the reservation for the MARGINAL unit, which a household equalising value across categories prices near the going price — small, and larger the tighter its commitments, e.g. `premium = stapleSpendShare`, ~0.35. **Both are derived; they disagree by 100x, and the existing number is 0.019, so either one is a large move in the stage that sets every consumer price.** The step-bid shape is what makes them differ: a single `(units, maxPrice)` pair cannot express a demand curve, so whichever number goes in is standing in for a schedule. **NOT LANDED DELIBERATELY** — this is the second change (with the wage level) that genuinely wants a measurement either side of it, and the right fix is probably neither number but a household bid that is a BUDGET rather than a quantity at a ceiling, which is a larger change than COH4 scopes. Owner: COH4. |
| **A RETIRED BANK FACILITY REACHES NO LENDER (§7.198)** | Opportunistic deleveraging in stage 08 retires whatever is economic to retire, and a drawn FACILITY on that list leaves the issuer's ladder with no lender repaid: the holder register settles market paper only, filtering facilities out. **Do not fix it by paying the bank alone** — the facility is also an itemized loan on that bank's book (G2), and moving the cash without shrinking the asset breaks the per-bank identity, which is precisely what happened when it was tried mid-sweep. One change to both sides, together. **Owner: G2.** |
| **A DISCOUNT BILL'S YIELD REACHES NOBODY'S CASH (§7.197)** | The bill books clear a YIELD but the buyers pay FACE and are repaid FACE, so a bill is bought at par, redeemed at par, and returns nothing whatever it cleared at. Stage 11 used to discount the treasury's proceeds against that, which charged the government for a discount no buyer received; with PUB's treasury auction the government is paid what the buyers actually paid, which is consistent but leaves the instrument's economics wrong. **Owner: the bill book's price/face split** — until it exists, a bill's cleared yield is a statistic, not a cash flow. |
| **THE GOODS-PAYMENT PATH PASSES BUYER-LOCAL MONEY AS `amountUSD` (§7.197)** | In `05-unit-bidding.ts` the ex-works and freight legs pay `l.units * exWorksBuyerMoney` — a figure in the BUYER's local money, produced by `convertLocal` — straight into `pay({ amountUSD })`, while the trade invoice beside them converts the same quantity with `localToUsd`. One of the two is wrong and they disagree by the buyer's FX rate (harmless only for USA, at 1.0). Found while routing trade credit and left alone deliberately: picking a side is a currency-units audit of the whole stage, not a line to patch, and patching the wrong one silently rescales every goods payment in three regions. |
...
178. **CAP/rule 3 — the capex categories' demand was a FROZEN SEED PLACEHOLDER beside the firms'
    real capex. Investment was represented twice and this is where the second copy lived.**
    - `03-category-demand.ts` carried the level forward UNCHANGED for the five capital-goods
      categories, by design and with a comment explaining it — the real demand was each firm's own
      capex, bid directly in stage 05. **But a frozen level is still a representation**, and it is
      the one that sizes the industries: measured 54.0B/yr built against 83.6B/yr bid, **1.55x**,
      with four of the five categories in permanent shortage as a result (§7.168).
    - **The level IS the firms' own capex now** — a measurement of what they will actually bid,
      not a statistic standing beside it. `corporateDemandUSD` stays absent for these categories:
      stage 05 sizes those bids from each buyer's capex directly, and adding a generic corporate
      share on top would be the double count the original comment correctly warned about. **The
      comment was right about the bids and wrong about the level.**
    - **Measured: 54.0B → 76.9B sized against 98.9B bid — the gap narrows 1.55x → 1.29x.** Harness
      green, 0 violations.
    - **It does not close entirely, and the residue is the SEED.** The industries' CAPACITY was
      built off the old frozen number and capacity moves slowly, so the level being honest fixes
...
197. **OWN7/CASH/PUB — the UNMODELED boundary goes from twenty-five reasons to six, and breaks.**
    - **The question that started it: "why are there still holders the model doesn't name?"**
      §7.196 printed the boundary by reason for the first time, and the answer was ugly: 25 lines,
      the four biggest of them `<book> dealer inventory` — securities bought from, or sold to,
      nobody, at 16B gross over ten weeks.
    - **THE FLOAT. 07b and 07d still auctioned the WHOLE outstanding**, on OWN2's claim that the
      instrument was "already net of what does not trade in it". Measurement falsified it. 07f had
      named its non-bidders one at a time, which is not the same as naming every participant, so
      the share no named book holds stayed in the float. All five stock books now use the rule 07c
      and 07e already state: **the float is what the participants in THIS book hold between them.**
    - **THE SEED WAS MINTING CLAIMS BEFORE WEEK 1.**
      `INSTITUTIONAL_OPENING_BOOK_SHARE.corpBond = 0.45` sized the opening credit book at 45% of a
      debt stock counting every tranche — floating, bank facilities, commercial paper, public and
      private — and placed the whole of it on public FIXED paper only. Institutions opened holding
      **132% of the USA corporate bond stock** (127 EUR, 126 UK, 120 JPN). The desks hid it by
      going short into the boundary. A bond book has no unnamed holder to leave room for, so the
      institutions open with the whole tradable stock, and the candidate lists carry exactly the
      instrument 07b and 07d clear. The sovereign slice went the same way: measured, the named
      books held **79.6%** and 137B of USA paper belonged to nobody — the institutions take
      whatever the central bank and the capital-constrained banks leave, computed after the bank
      pass rather than stated in front of it.
    - **THE RESIDUAL DEALER IS DELETED.** Every book handed `tradableFloatUSD - allocatedUSD` to a
      dealer funded by nobody. That looked like market making while the float was the whole issue;
      once the float was what the participants hold, it was plainly a counterparty the model does
      not name. The five stock books ration **BOTH SIDES** now (`unsoldStaysWithHolder`): buyers
      take no more than the sellers offered plus what the issuer brought, sellers place no more
      than the buyers want, and the larger side is scaled pro rata — the identical treatment an
      oversubscribed book always gave its buyers. **A seller that finds no buyer keeps its paper**,
      which is a far more honest illiquidity than an invisible bid. The flow books (fx-clearing,
      fx-hedging, repo, 07g) keep the residual: there the float is an inelastic ORDER from outside
      the participant set and the unabsorbed remainder is a real measurement.
      - **The flag has to travel to the worker pool.** The worker rebuilds the packed struct field
        by field and hands it to the kernel as `never`, so a parameter left behind there runs a
        DIFFERENT market in the parallel path with nothing to catch it.
    - **Then the rest of the boundary, one pair at a time.** Each was two halves of one payment
      with neither half pointing at the other:
      - **Corporate treasuries MINTED their bills.** Stage 08 closed the gap between the sleeve and
        the book by pushing a holding onto the company and paying nobody — rule 13 and rule 1 in
        one block, and the reason 07f had to carve them out as a holder that never bids. They bid
        now, the most price-INSENSITIVE schedule in that book, and honestly so: a treasurer parks
        cash because it has cash, not because the yield tempted it.
      - **Trade credit** already knew both its parties. Credit extended moves seller → buyer in 05,
        collection moves buyer → seller in trade-settlement. The invoice is also cut to EX-WORKS:
        the freight inside the landed price belongs to the carrier and was paid to it at shipment,
        so carrying it on the receivable had the seller lending money the carrier had already taken.
      - **Principal, premiums and buybacks.** The register credited redemption cash straight onto
        the holder's book while stage 08 posted the issuer's side to the boundary — and the CALL
        PREMIUM was paid TWICE, once to the holders of record and once to nobody.
      - **The primary market** crossed the boundary in both directions at once: the clearing house
        paid UNMODELED for the paper the book took while stage 08 posted the issuer receiving the
        same proceeds from UNMODELED. The CCP pays each issuer by name now; the lead's fee and
        residual are payments, not a direct write on its reserves.
      - **The bank net income accrual was settling.** Every line of a bank's P&L is already booked
        against `bankEquityUSD` in the sector ledger, so the bridge credited the same income twice.
    - **A MATURING SOVEREIGN PAID THREE HOLDERS NOTHING.** Foreign institutions were skipped by an
      `entity.region !== regionId` filter though the row inside already tests `h.issuerRegion`; the
      desks' own sovereign and bill inventory was never redeemed, so a desk kept a claim on paper
      that had matured; and the corporate treasuries were not repaid at all.
    - **PUB — THE TREASURY AUCTIONS ITS PAPER.** Stage 11 credited the whole issue to the TGA the
      moment it was written, and under OWN7's float that paper could never be bought: outstanding,
      held by nobody, and 51B a week paid on maturity to a creditor who was never there. Unheld
      sovereign paper is a **primary offering** — which is what a treasury auction is — priced in
      the same solve as the outstanding stock; the clearing house pays the treasury for what the
      week's demand takes and what nobody takes is offered again. **And what still finds no buyer
      by the time it matures is simply gone**: a debt nobody holds is owed to nobody, and the
      treasury was never paid for it either. `lastUnsoldMaturedUSD` measures it.
    - **THE BOUNDARY IS A HARNESS VIOLATION NOW.** `{ kind: 'UNMODELED' }` absorbs whatever is paid
      to it and pays whatever is asked of it; from the inside that is indistinguishable from a leak,
      and nothing checked it. `BOUNDARY_FRONTIERS` declares every reason that may still cross, with
      the project that owns it; **anything else fails the harness on the week it appears.** It is a
      list to shorten, never a place to add a line to make a run pass. It caught the bank accrual,
      the sovereign seed and the sovereign redemption on its first run.
    - **Two more defects the checks found:** firm commitment created the tranche at the MARKET TAKE
      while the lead was paid for and held the residual, so the lead's desk held a claim on paper
      that did not exist (`issuedUSD` now issues the whole deal); and the holdings-ledger check
      scored a WORKOUT IN PROGRESS as minting claims — a defaulted issuer's paper left `outstanding`
      the week it defaulted while its holders' rows stayed until the estate distributed. A defaulted
      bond is still a claim until it is written off.
    - **MEASURED, 10 weeks, against the pre-session baseline (890 damper / 11.0B bypass / 12 viol):**
      - **The boundary is SIX declared frontiers**: non-auction operating receipts 6.7B and
        non-auction opex -2.3B (the goods market's own edge), commercial paper (no cleared book —
        the money funds are its real buyer), warehousing, the carriers' lane split, G5's residual
        claimants. Every securities, cash, principal, premium, invoice and primary flow now names
        both of its parties.
      - Clamped overdrafts 3.7B → ~0.3B; the holdings-ledger check silent; TGA 2.9B..31.0B, never
        negative, so the auction funds the deficit the direct credit used to.
      - **DAMPER 890 → 1030 persistently bound. A REGRESSION in §6.1, and the honest shape of this
        whole sweep**: floats are what holders actually hold, the treasury's supply is real and
        one-way, and neither is absorbed by a dealer nobody named. **The pressure was always there
        — the residual dealer was hiding it.** Do not widen the damper. The owner's question is now
        a well-posed one: who buys when the holders as a group want less than they hold?
    - **Recorded, not fixed:** a discount BILL's proceeds. Its buyers pay face in the clearing books
      and are repaid face, so its cleared yield reaches nobody's cash; the auction pays what the
      buyers paid. Owner: the bill book's price/face split. And the goods-payment path passes
      buyer-LOCAL money as `amountUSD` while the invoice converts it — one of the two is wrong, and
      finding which is a currency-units audit of stage 05, not a line to patch.

198. **CP and XB3a-2 — the last unpriced book, and the carriers' missing cash leg.**
    - **"Why is commercial paper not cleared?"** Because it arrived with the bills in WS5 and got
      the ISSUER half of a market and none of the buyer half — its buyers, the money funds, did not
      exist until WS7. So the paper was priced at `cleared 13-week bill + the issuer's short-horizon
      expected loss + 15bp`, and three things followed, each of them something rule 1 forbids
      everywhere else:
      - **The SIZE was the issuer's alone.** A treasurer computed its working-capital gap and the
        ...
        that name at that level, so the market could not ration by size — only by a price it did
        not set.
      - **The market's only voice was a BINARY GATE.** A rating in the club and a PD under 8% meant
        ...
        stress does not arrive as a switch — it arrives as "it rolled, but forty basis points
        wider, and only sixty per cent of it", and that state was inexpressible.
      - **The HOLDER had no name.** Cash for every issue came from `UNMODELED` and went back to it
        at maturity — the boundary line §7.197 could not close from the settlement side, because
        the instrument had no book to clear in.
    - **A fourth, quieter and worse.** CP is a FIXED tranche, and stage 08's coupon accrual filtered
      on `rateType === 'FIXED' && !isBankFacility`. **The CP coupon was being paid to the CORPORATE
      BOND holders of record** — a register whose float explicitly excludes CP.
    - **It clears now**, as its own instrument type with its own register, its own place in the
      estate waterfall (senior unsecured, ranking with the bonds — and its holders are money funds,
      which is exactly why a CP default is systemic and a bond default usually is not) and its own
      row in the ledger check. The issuer brings a **primary offering** sized by the same real
      working-capital gap; **its walk-away is the REVOLVER**, because no treasurer pays more for
      paper than the committed line beside it costs, so the funding squeeze the rating gate
      asserted is now the market pricing past the alternative — and it can be partial. The buyers
      are the money funds and the cash sleeves that already run through the bill and repo books,
      plus the banks' own desks; a buyer's reservation is **its own alternative** (the cleared
      13-week bill) plus the loss it expects on THIS issuer over the paper's actual life.
      `CP_LIQUIDITY_PREMIUM_BPS`, `CP_ACCESS_RATINGS` and `CP_MAX_ANNUAL_PD` are all deleted:
      the premium is whatever the book clears over bills, which is what a liquidity premium IS,
      and credit policy is a SIZE, never a veto.
    - **THREE DEFECTS IT EXPOSED, and the first is the important one:**
      - **A DESK'S PRIOR POSITION WAS CONDITIONAL ON THIS WEEK'S FLOAT.**
        `buildDealerDeskParticipants` declared a desk's holding only for names with a live float —
        and OWN7 walked straight into it. Once the float is "what the participants hold", a book
        that sets it AFTER the desks are built hands them zero, and `applyDealerDeskFills`, which
        rebuilds the book from the fills for every name it cleared, then deletes the position with
        no cash leg. **That is the WS5 bug the file's own comment warns about**, re-entering
        through an ordering nobody had reason to think about before OWN7. Measured on the CP book's
        first run: a 2.34B desk position entering the week as 0.02B, and 2.3B of paper held by
        nobody. A position is a FACT; it is declared before any float test.
      - **Every book was sizing its desks against the PRE-SHRINK float.** 07b, 07d, 07e and the
        bill book all left `tradableFloatUSD` at the whole outstanding until after the desk build,
        so each desk got capacity against an issue that is not for sale. All five now set the
        holders' half of the float first and add the desks' own books after.
      - **Opportunistic deleveraging was retiring CP.** The surplus-cash prepayment beside it
        already said "CP is 07f's to resize against the real gap"; this path did not, and the
        register that repays holders on a ladder change filters CP out — so retired paper left its
        holders with a claim on nothing and no cash. Caught by the new ledger row within one run:
        EUR holders at 109% of the CP stock by week eight.
    - **"Why are carriers not built into the corporates framework?" They ARE** — real companies
      since XB3a-2, with real fleets, real fuel at the refined-product market's own price, real
      crew through the labour market, listed equity, a home bank and a P&L profile in stage 08.
      What was never wired is the CASH: the buyer's freight leg paid the boundary, and the
      carrier's freight then arrived on its own books as `non-auction operating receipts` — also
      from the boundary. **Two anonymous ends of one payment whose parties are both known.** The
      buyer pays the carriers on its lane now, split by the share the freight auction gave each.
      It is also ONE quantity (rule 3): a carrier's revenue was re-derived from
      `shippedTonnesByLane x rate x share` in the CARRIER's money while the buyers were charged
      landed cost in THEIRS — a second computation of the same freight that could not agree with
      what anybody paid. What a carrier earns is what its customers paid it.
    - **MEASURED, 10 weeks.** The boundary is **FOUR reasons**, from twenty-five when this sweep
      began: `non-auction operating receipts` 6.1B and `other opex beyond auction settlements`
      -2.4B (the goods market's own edge), warehousing, and G5's residual claimants. Every
      securities, cash, principal, premium, invoice, primary, freight and short-paper flow in the
      model now names both of its parties. CP: 623 issuers, 16.1B outstanding, clearing ~20bp over
      the cleared 13-week bill, no roll failures in ten weeks. Damper 947 (890 pre-sweep; it peaked
      at 1042 before the desk-position fix, which is how that bug was found).
    - **Recorded, not half-closed:** a drawn FACILITY retired on the deleveraging path reaches no
      lender either. Paying the bank is not enough on its own — the facility is also an itemized
      loan on that bank's book (G2), and moving the cash without shrinking the asset breaks the
      per-bank identity, which is exactly what happened when it was tried. One change to both
      sides, together. **Owner: G2.**

199. **SUPPLY/CHAIN and COH — the seed had a circle in it, and the household balance sheet had one
    number doing four jobs.** Six changes, foundation tier, in work-order sequence. **NONE OF IT IS
    MEASURED** (the user directed the work to proceed without a run); what each owes a measurement
    is stated at the end.
    - **THE SEED'S INVESTMENT FIXED POINT, CLOSED.** A firm's revenue is derived from its primary
      category's demand seed and its capex from that revenue — while `I`, which sizes the
      capital-goods half of that same demand seed, is the sum of exactly those capexes. The seed
      resolved the circle by using a **different number on each side**: the GDP-share placeholder
      for the demand that sizes the firms, then the authoritative vector built from their real
      capex, which overwrote it. So the capital-goods industries were built for one investment
      number and asked to supply another — 1.29x more bid than built for, four of five categories
      short at 65-174% over base, plant shrinking because nobody can make the machines, and
      through the cost of capital a share of the ~29% unemployment blocking unrelated work
      (§7.168, §7.178, §7.179).
      - `seedRegionCategoryDemand` is extracted so the vector can be built more than once, and
        `solveSeedInvestmentFixedPoint` iterates the two against each other to 1%. The map is a
        strong contraction — capital-goods makers are a slice of the economy, so a change in their
        demand moves total capex by much less than itself.
      - **The RNG is rewound to the stream position as it stands before the first generation**,
        not to the seed, because the region and FX builders draw from it. The surviving universe is
        bit-for-bit what a single generation against the converged vector produces, so the extra
        passes relabel nothing (rule 10). That detail is the whole reason this was safe to do.
      - Second half, same circle from the allocation side: `dealProductLinesAndHeadcount` is
        extracted and RUN AGAIN against the authoritative vector. That half converges in one pass
        on its own — a firm's revenue, PP&E and capex are set before any line is dealt, so `I` does
        not move when the lines do — and it draws no RNG.
    - **TWO PLAN ROWS WERE STALE AND ARE CORRECTED HERE.** §7.177's capacity-off-PP&E was already
      RESTORED in the tree (it landed with §7.180's commit), and DEM (item 5) closed with §7.181.
      Neither §4 row said so. Read the code before believing a row's state.
    - **COH1 — SPENDABLE NOW AND ACCUMULATED ARE TWO STOCKS.** `accumulatedSavingsUSD` drove the
      deposit split, the equity-like split, the private-business split and the institutional-claims
      split — every asset class at once — off a stock that tracked only the saving FLOW and never
      where the saving went. **A tier that put everything into a house and a pension looked exactly
      as liquid as one holding cash**, and the buffer rule that decides the savings rate had
      nothing real to be a buffer of. Deposits follow the liquid stock now, the invested stock
      backs the rest, and `accumulatedSavingsUSD` survives only as their derived sum.
      - **Dissaving drains the buffer first** and only what it cannot cover comes out of the
        invested stock. A household spends its cash before it sells anything, which makes forced
        selling (§7.166) the END of a squeeze rather than its beginning — a mechanism that could
        not exist while both stocks were one number.
      - And it makes the wealthy-hand-to-mouth middle CAUSAL rather than coincidental, which is
        what §7.145 derived and could not explain.
    - **COH2 — THE PENSION CONTRIBUTION IS THE SAVING HOUSEHOLDS ALREADY DECIDED TO DO.**
      `PENSION_CONTRIBUTION_RATE = 0.09` is gone; its own comment carried the exit condition and
      DEM met it. The life-cycle half of saving — disposable income times the retired share of the
      population, the rate at which the working population must set aside income to support the
      population that is not — was being accumulated into the household's own stock **while a flat
      9% of the whole sector's income went into the pension funds beside it.** Two representations
      of one motive, and the stated one was larger. The stage collects the measured flow, squeezed
      by each cohort's own budget: a cohort that cannot save contributes nothing, which is what a
      contribution holiday IS.
      - It also corrects COH1's split, made an hour earlier: **a saving flow divides by MOTIVE, not
        by a portfolio weight.** `equityExposureShare` goes back to its proper job — the
        risky/cautious split of the invested STOCK.
    - **COH2 — AND A PENSION FUND OPENS AS BIG AS WHAT IT OWES.** `beneficiaryLiabilityUSD` was
      reversed weekly but the SEED still fell back to `totalAssets − equityCapital`, which is
      precisely the circularity `INSTITUTIONAL_OPENING_BOOK_SHARE`'s own doc names as the reason it
      survives. The stock now follows from the age structure and nothing else: in a stationary
      population it is the contribution FLOW times how long a contributed dollar stays in the
      system, `(workingLife + drawdown) / 2` averaged over contribution ages. The fund's capital is
      its SURPLUS against what it owes. `INSTITUTIONAL_CAPITAL_RATIO` gets an owner on the way.
    - **COH3 — the identity assert is deleted, because it could not fail.** Every term on both
      sides of `assertHouseholdIncomeIdentity` came from the same four constants; it was a
      tautology whose real effect was to pin the seed to an identity the model stops using in
      week 1.
    - **WHAT THE NEXT MEASUREMENT RUN MUST READ, in this order:**
      1. **The capital-goods fill rate**, against §7.168's 8% and the per-category table. That is
         what the fixed point exists to move, and everything else in the foundation tier is
         waiting on it.
      2. **Unemployment**, against the ~29% that has been binding. The fixed point should relieve
         it from the supply side; if it does not, the attribution in §7.179 is wrong.
      3. **The household savings rate and the liquid/invested split**, which COH1 and COH2 both
         move, in opposite directions on the same flow.
      4. **Pension fund size and funding ratio**, which now open on a derivation rather than on
         their own holdings.

200. **HSG — the last two constants in the housing row, and a third row that was already done.**
    - **THE PRICE ALREADY CLEARS, and the §4 row still said it did not.** The walked index, its
      [0.5, 3.0] bound, the credit factor and the `|| 400000` fallback are gone; the price is what
      the marginal buyer can pay, walked down the wealth tiers' own affordability and floored at
      the construction sector's cleared build cost. Third stale row of the day (§7.199 found two);
      the lesson is the same one: **read the code before believing a row.**
    - **EVERY BANK QUOTES ITS OWN MORTGAGE NOW.** `MORTGAGE_SPREAD_OVER_10Y_BPS = 170` had every
      bank charging every borrower the same spread — in a file whose own `BankLoan.marginBps` doc
      says a margin is "quoted by the bank's own credit arithmetic at origination, the same
      expected-loss + capital-cost pricing the bond market uses". The household book uses that
      arithmetic: the loss rate this bank's OWN vintages are running (frequency x the severity its
      own LTV cross-section implies), the mortgage risk weight, its own cost of equity, and a
      servicing cost that gets an owner on the way. **A bank whose book is underwater quotes
      wider**, so a credit tightening reaches the housing market as a RATE rather than as the
      stated credit factor the price walk already deleted.
      - The region keeps the KEENEST quote, because a borrower shops, and that is what the
        affordability walk reads. The flat spread survives only as the seed's opening quote.
    - **AND TURNOVER IS AN OUTCOME.** `HOUSING_TURNOVER_RATE_ANNUAL = 0.04` carried its own
      rule-4/13 objection and was fixing volume on BOTH sides — the supply the price walk absorbs
      and the demand the banks lend into. Two derivations replace it:
      - **A tenure ends.** Every owner sells once, the estate doing it if the owner does not, so
        the floor is one sale per tenure — and a tenure is what the Gompertz hazard says an owner
        of the region's median ADULT age has left. An ageing region turns its stock over faster.
      - **Or moving up becomes affordable.** An owner trades up when today's income at today's
        quote supports more than it borrowed, which is a measurable share of the vintage
        cross-section because every vintage remembers the house it was written against. Those
        owners move once over the same tenure: `(1 + tradeUpShare) / tenure`.
      - So turnover rises as rates fall and falls back to the forced-move floor as they rise, and
        **no coefficient decides the sensitivity — the vintage cross-section does.**
    - **Rule 3 on the way:** the affordable loan was computed twice in the same function, once for
      the turnover reading and once for origination. Hoisted to one.
    - **UNMEASURED**, like everything in §7.199. What it owes a run: mortgage rates that differ by
      bank, turnover that moves with the market, and the wealth effect riding on a cleared price —
      §5-HSG's own three verify conditions, none of which had anything to read before today.

201. **CRD, DER and ETF2 — the CDS book, two clamps, a stated volatility, and the overdrafts.**
    Five changes across the markets and depth tiers, still **UNMEASURED** (§7.199's directive).
    - **SINGLE-NAME CDS CLEARS (CRD/DER2), and the second cross-market basis appears.**
      `comp.cdsSpreadBps` was `oasSpreadBps + a random draw in [-4, +4]`, bounded to [10, 5000] —
      a decoration on another price with a clamp on each end, rule 1 and rule 15 in two lines.
      Nothing traded it and nobody was on either side, so **a bank could not lay off a credit
      concentration at all**: the only way to reduce one was to stop lending, and the whole point
      of the instrument is that it separates who CARRIES the risk from who FUNDED the loan.
      - Framed like every other book here: the position priced is SELLING protection, an unfunded
        long in the credit. **The float is the protection somebody NEEDS** — the exposure a
        lender's capital does not let it carry against one name, measured against the
        large-exposure limit. **That is the consumer `09-concentration-risk.ts` has never had**
        (§5-CRD: it computes those flags every week at 8.5% of run time and nothing prices off
        them). The participants are the sellers, whose reservation is what the same credit costs
        them to carry — the corporate bond book's own arithmetic, because it is the same risk.
      - A credit event pays par less what the workout actually recovers (G5, §7.192), so
        protection is worth what the estate says rather than a stated fraction.
      - **The cleared spread less the cleared cash OAS is the CDS BASIS**, published per issuer.
        Second agreement test between two of this model's markets after DER1's swap spread; a
        persistently negative basis says the cash market is paying more for the same default risk
        than the synthetic one.
    - **THE RATIO CLAMPS ARE GONE, and the reason they existed is answered where it belongs.**
      Leverage was bounded to [0, 100] and coverage to [-50, 50] because EBITDA passes through
      zero and `debt / EBITDA` explodes — but the denominators were already floored, so the
      numbers are finite without the bounds, and **what the bounds destroyed was the fact that
      produced them**: a firm at "leverage 100" and one at leverage 8 came out of the rater
      identical. §5-CRD named the honest place, and it was right: with no earnings the two ratios
      carry no information, so the rating rests on liquidity — a firm that can cover its debt from
      cash and its committed line is not yet distressed, one that cannot is. The seed rater is
      told the same fact (§7.4).
    - **AN OPTION IS REPRICED AT THE NAME'S OWN VOLATILITY (DER).** `pos.impliedVol || 0.3` put a
      flat 30% on every option whose row lacked one, so a Black-Scholes price computed from it was
      a stated price and no name could be riskier than another. Until an options BOOK exists to
      imply a vol from, the honest input is the underlying's own realised vol, falling back to its
      region's index — no constant anywhere in the chain.
      - **Rule 3 on the way:** there were TWO realised-vol estimators and one was private to stage
        05 with its own 0.16 fallback, so a market with too little history reported as sitting
        exactly at its baseline — which reads as "no excess vol" whether that is true or unknown.
        One estimator now, with no fallback baked in: unknown returns undefined and each caller
        says what it does about that.
    - **ETF2 — THE OVERDRAFTS, and the plug that was paying for them.** The harness's largest
      remaining violation family. Two causes, both in the ETF flow, and both are the same doctrine
      every other book already applies:
      - **The cash test was in the wrong place.** `min(want, investor.cashUSD)` is the right test,
        applied inside a loop over FUNDS — so an investor buying into three of them was allowed
        its full balance in each and **the same dollar was budgeted once per fund.** A running
        budget now, opening at cash plus what the week's clearing books have already committed.
      - **Nothing checked that a fund could pay a redemption.** Settled in CASH it is money leaving
        the fund, and a fund whose assets are securities paid out anyway. Rationed by its own cash
        on top of the AP's capacity — the tighter constraint binds — and what cannot settle is
        unmet flow, which this mechanism already meters.
      - **Recorded rather than pretended:** the real answer is IN KIND. An ETF redemption hands
        over the basket, not money, and needs no cash at all. Next slice of the row, and the reason
        the cash cap is a constraint rather than a fix.

202. **G5's two remainders, and HF — an equity SHORT that borrows, locates, recalls and squeezes.**
    - **A FIRM DRAWS ITS COMMITTED LINE BEFORE IT DEFAULTS (G5).** A revolver is a promise the
      lender cannot withdraw, and the model let a firm run out of cash beside an undrawn one.
      `committedLineHeadroomUSD` sizes what the covenant still permits — the interest the firm's
      EBIT can cover at `COVENANT_INTEREST_COVERAGE` over what it already pays, capitalised at the
      revolver rate — and it is drawn before the default test, so a default is now a firm that ran
      out of *committed credit*, not one that ran out of cash on a Tuesday. `COVENANT_INTEREST_
      COVERAGE` got one owner in the process; 07g imports it rather than restating it.
    - **PRIVATE COMPANIES CHANGE HANDS (G5).** A sponsor held its portfolio for ever, so the
      capital-recycling loop was open at one end: LPs were called and never distributed out of a
      sale. A company held past `PE_FUND_LIFE_WEEKS` is sold to another sponsor with the dry powder
      to buy it, at the same `equityValueUSD` mark the balance sheets already carry, with the
      capital called from the buyer's LPs and distributed to the seller's. Two named parties, one
      price, both legs in the same pass.
    - **HF — THE SHORT WAS A LABEL ON AN ENUM.** `'LONG_SHORT_EQUITY'` has carried a comment saying
      it needs a real short to be whole since the funds were built. Nothing in the model could
      express a participant who profits when a price falls, so every equity schedule sloped the
      same way and the only bearish act available to anyone was to own less. **A short is three
      obligations, and each of them is now somewhere real:**
      - **BORROW.** The lender's shares move to the borrower against cash collateral at the market
        value, plus a fee. Both legs are money between two named parties, so a fund cannot short
        what it cannot fund — its own cash, its unsettled receipts, and its prime-brokerage line.
      - **LOCATE.** The borrow is an auction on the same engine as every other book: the float is
        the borrow DEMAND, the participants are the holders who will lend, and their position in
        this book is what they have out. A borrow the auction does not fill is a short that does
        not happen, which is what "hard to borrow" means once it is an outcome. **The cleared
        price is the fee, in annual bps of market value.**
      - **RECALL, and the SQUEEZE.** Lending does not shrink a lender's position — the shares
        leave, the exposure does not — so a position below what it was when the loan was struck is
        the lender selling out from under it, and that much comes back. The recalled borrower's
        buy-in enters 07e as a **mandated core with no reservation price**, because the obligation
        is to produce shares, not to get a good price for them. Forced buying lifts the print,
        which widens every other short in the name and reprices the borrow auction against a
        lendable base that just got smaller. **Nobody wrote that loop down; it falls out.**
    - **NO NEW FREE PARAMETER IN ANY OF IT.** A lender's reservation fee is what the loan costs it
      to carry — the capital the one-week gap consumes at its own required return, with the gap
      read from the name's own realised volatility (§7.201's single estimator) and capped by the
      equity book's own damper where there is too little history. A fund's short size is the mirror
      of the long schedule it already runs: full size at the same distance ABOVE its own fair value
      that it takes full size below it. The disagreement about value that gives the equity book its
      slope, read from the other end.
    - **BOTH SIDES CARRY THE POSITION.** `stockLoanNetUSD` nets a party's contracts the way
      `repoLentUSD` nets the repo book: shares owed back less collateral held, or the mirror. Zero
      the week a loan is struck and the short's running P&L every week after — which is the number
      that would otherwise have been nowhere. `shortInterestShares` per name is a measurement of
      the same book.
    - **THE LEDGER STAYS CONSERVED, AND THAT IS THE POINT.** A borrowed share is delivered, not
      duplicated: the lender's rows fall, the borrower's rise, and the borrower sells them into
      07e's real bid. `HoldingsStore.addShares` is the new primitive that lets a transfer happen
      outside an auction, and it draws a withdrawal across a position split over several rows so no
      row goes negative. What 07e needed in return is two facts: a lender's ceiling comes down by
      what it has lent (or it walks straight back in to re-buy what it just lent), and a recalled
      borrower's delivery obligation is a floor with no price attached.
    - **UNMEASURED**, like everything since §7.199. What it owes a run: whether any borrow clears
      at all (a fee auction with no lenders is a dead ceiling, and the harness asserts on those),
      what the fee distribution looks like across names, and whether a recall cascade is a squeeze
      or an oscillation.

203. **DER closes: a futures curve that clears, and the corporates finally in the hedging book.**
    - **THE COMMODITY CURVE WAS SPOT TIMES A CONSTANT (DER4).** `futures1M/3M/6M` were
      `spot x exp((r − convenienceYield) x T)` with the convenience yield seeded once and never
      touched again. So the curve could not back off when this model's own commodity market went
      short, could not build out when it went long, and nobody was on either side of it — three
      prices with no market behind them (rule 1) and a shape stated before it was produced (rule
      13). **The convenience yield is what a traded curve IMPLIES, and here it was the input.**
    - **BOTH SIDES OF THIS BOOK ARE INDUSTRIAL, AND BOTH WERE ALREADY IN THE WORLD.** The natural
      short is a producer selling forward the production it will have; the natural long is a firm
      whose recipe draws the commodity, measured off its real product mix through
      `CATEGORY_INPUT_REQUIREMENTS`. **Which side is bigger sets the SHAPE**: backwardation and
      contango stop being labels on a formula and become the outcome of who needed the hedge more,
      which is what they are. The convenience yield is inferred from what the 6M cleared at.
    - **THE STORAGE DESKS HOLD THE TOP OF THE CURVE, and that is not a clamp.** A print above spot
      plus financing plus real storage is free money to anyone who can hold the physical, so the
      desks bring supply into it. Rule 15's distinction exactly: a participant's price, not a
      bracket around someone else's. `PHYSICAL_STORAGE_COST_ANNUAL` is a property of the SUBSTANCE
      — energy boils off, metal sits in a shed, grain rots — which is the kind of primitive rule 4
      admits; what it forbids is a real-world outcome, and none of these is one.
    - **THE CORPORATES WERE MEASURED AND COULD NOT ACT (DER5).** XB3a-5 built transaction FX
      exposure properly — a firm delivers, waits to be paid in somebody else's money, and the cash
      that eventually moves is a currency's worth away — and then nothing could hedge it, because
      the forward market had one client population and it was the fund managers hedging
      TRANSLATION risk on foreign holdings. Corporates now bid in the **same** auction against the
      **same** desk capacity, so a corporate hedging widens the basis for the fund managers. That
      is what a shared dealer balance sheet means, and it is why they had to be IN this book
      rather than beside it.
    - **ONE OWNER FOR "HOW MUCH VARIANCE HAS THIS BALANCE SHEET ROOM FOR."** `exposureToHedgeUSD`
      lives beside `committedLineHeadroomUSD` and `COVENANT_INTEREST_COVERAGE` in
      `corporate-financing.ts` and has three callers: the committed line, DER4's commodity hedgers
      and DER5's currency ones. It is the same test 07g applies to decide which corporates must pay
      fixed, read against a price or a currency instead of a rate. **No new free parameter in
      either slice** — what a hedger will give up is the capital its unhedged exposure consumes at
      its own cost of capital, one formula with two signs.
    - **RULE 3 ON THE WAY:** the commodities screen redrew its own smooth `exp()` curve through the
      stated yield while the traded curve sat on the commodity beside it. Two representations of
      one thing; it reads the cleared one now.
    - **UNMEASURED.** What it owes a run: contango when inventories are high and backwardation when
      scarce (both genuinely occur in stage 07), expiry convergence within the dealer spread, and
      §5-DER's own test — that hedged producers and hedged exporters demonstrably feel less P&L
      from the shocks they hedged, which is the whole point of items 4 and 5.

204. **COH4's savings destination: the split was already being made, one file away.**
    - **`HOUSEHOLD_SAVINGS_TO_DEPOSITS_SHARE = 0.3` HAD ALREADY CONVICTED ITSELF.** Its own comment
      said so: *"where a household's saving goes is a portfolio choice it should make."* What
      changed is that COH2 (§7.199) started making exactly that choice one file away and nothing
      connected them — the saving flow was split by MOTIVE into a spendable buffer and a life-cycle
      contribution, which IS the deposit decision, while the banking stage went on multiplying the
      gross flow by 0.3.
    - **So the constant is the SEED and nothing else (§7.4).** `householdState.liquidSavingShare` is
      the measured share, written where the split is decided, and both readers — the money fund's
      competition denominator and the deposit inflow it is measured against — take the same number,
      which is the §7.5 duplicated-constant shape closed at the root rather than kept in sync.
    - **What it buys:** the banks' funding competition now responds to households changing WHY they
      save. An ageing region contributes more to pensions and less to deposits, so bank funding
      tightens and the deposit rate rises — a channel that could not exist while the split was a
      constant, and one DEM's age structure already produces the input for.
    - **THE OTHER HALF IS DELIBERATELY NOT LANDED, with both candidate derivations recorded in
      §6.1.** The household bid premium's two honest derivations differ by two orders of magnitude
      and the number they replace is smaller than either; the real defect underneath is that a
      one-step `(units, maxPrice)` bid cannot express a demand curve, so whichever number goes in
      is standing in for a schedule. That is a bigger change than COH4 scopes and it wants a
      measurement either side of it, like the wage level.
    - **UNMEASURED**, like everything since §7.199.

205. **Three rows closed in one pass: the distribution tier, the credit-tier join, and the ETF's
     basket.**
    - **IND16 — THE TRAP WAS THE DESIGN.** The row had been open with a warning on it: a household
      already buys `facilities_and_logistics` AS A GOOD, out of its own buyer-mix share, so a
      channel margin on top of every other good would have paid the same sector for the same work
      twice — and neither number would have looked wrong, because both are real revenue with a real
      payer. **So the household's distribution spend MOVED.** The buyer-mix share is gone and the
      margin arrives on the goods it is actually spent distributing.
    - **THE MARGIN IS THE GOOD'S OWN PHYSICS, not a rate per category.** The channel holds the lead
      time it exists to bridge, capped by what the good will physically survive — a dairy cannot
      stock a quarter's cover at any price — and what that cover costs per dollar is the good's own
      carrying cost plus the money tied up in it at the region's short rate. Both terms already
      existed and both already encode value density and spoilage. **A service has no channel at
      all**: the physics does the exempting, and there is no list of exempt categories.
    - **THREE PRICE LEVELS, EACH WITH A REAL PAYEE.** Ex-works is what the producer received,
      `unitPriceUSD` is what a business pays delivered, and `shelfUnitPriceUSD` is what is on the
      shelf. **The CPI reads the shelf price**, because a consumer price index prices what a
      consumer pays; recipes and input costs keep reading the landed one, because that is genuinely
      what a firm pays. And the household bids the FACTORY GATE its willingness to pay leaves after
      the channel takes its cut, so a costly channel means less reaches the producer — the
      transmission the tier exists for rather than a bookkeeping layer.
    - **A FOURTH DECLARED FRONTIER CLOSED: warehousing has a seller.** `inventory carrying cost`
      was on the boundary because nothing in this model held goods for anybody. The distribution
      tier is that sector: it holds a firm's stock and is paid for it by name, with the revenue
      recognised in a pre-pass so a distributor's income does not depend on which firms happened to
      be processed before it. What still reaches the boundary is a region with no distribution firm
      at all, which is a fact about the region rather than a gap.
    - **CRD x COH — TWO PARTITIONS, ONE AXIS.** The credit tiers migrated on delinquency alone. The
      mechanism is right and stays, but two of its inputs were claims about the answer: four stated
      arrival multipliers (1.5 / 0.8 / 0.3 / 0.1) and a debt split by HEAD COUNT that had a
      subprime household and a super-prime one owing the same amount. **The join needs no mapping
      table** — credit tiers and wealth tiers are two partitions of one population, so they go on
      one axis: the BUFFER, months of its own spending a household could cover from what it holds
      liquid (COH1's stock). Rank the wealth tiers by it, lay the credit tiers along the same
      ranking worst-file-first, and each tier inherits the balance sheets in its band. A band with
      no buffer goes delinquent when its income moves against it and one with months of cover does
      not, so the ordering the multipliers asserted falls out instead of being written beside them.
    - **ETF2 — IN KIND IS WHY AN ETF CANNOT BE RUN ON.** §7.201 could only ration cash redemptions
      against cash the fund had, which was honest and was a constraint rather than a fix. An
      institutional redemption now hands over the BASKET: the fund delivers its pro-rata slice of
      everything it owns, securities and cash together, and no money has to be found because a fund
      that owns something can always deliver a fraction of it. Each book is sliced once for all of
      its redeemers at the same time; the delivered rows are real positions the constituent books
      price next week. **The household leg stays in cash** — a household cannot take delivery of a
      basket — so the cash ration now sits where it is genuinely a constraint.
    - **AND A CONSTANT THAT ARGUED WITH ITS OWN NAME IS GONE.** `AP_WEEKLY_CAPACITY_MULTIPLE_OF_
      EQUITY = 0.25` is a QUARTER of equity while the paragraph beside it argued a desk should turn
      over several times its equity; picking between them by hand would have been choosing the
      answer. An AP does not warehouse a basket, so what its capital must cover is the price move
      over the time it holds it — the equity book's own weekly cap, the same number the prime
      brokers haircut equity by. Capacity is equity over that move: several times equity, as the
      reasoning said, and it moves on its own if the book's volatility does.
    - **UNMEASURED**, like everything since §7.199. What this pass owes a run: whether the channel
      margin is a sensible fraction of a shelf price (it is unbounded above by construction — a
      good with a long lead time and a high carrying cost could carry a large one, and that is
