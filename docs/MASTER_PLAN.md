...
29-iv. **The defensive reads.** The 997 `??` (649) and `?.` (348) on values the types say are never
    ...
    fixed sixteen sparse stores). **a** domain, engine2, test and `App.tsx` (61), **b** the
    engine outside the simulation (232) and **c** the simulation (379) are DONE; what stands:
    **d** the UI 182 and the harness 143. One directory at a time (rule 10), the budget falling
    with each.
