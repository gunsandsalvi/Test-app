...
| 17 | depth | **ETF2 — a real price for ETF shares** | Prereq G3. **Its funding half landed early (§7.201) because it owned the harness's largest violation family:** one budget per investor across every fund it buys into, and a fund pays cash redemptions only out of cash it has. **Remains: (a) the share BOOK — pricing ETF shares against the APs' float instead of carrying them at NAV, which is the row's actual title; (b) IN-KIND redemption, which is what makes the cash cap unnecessary; (c) re-measure `AP_WEEKLY_CAPACITY_MULTIPLE_OF_EQUITY`, whose value contradicts its own comment by an order of magnitude.** |
