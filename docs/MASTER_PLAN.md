...
**THE RUN TO JUDGE AGAINST IS §7.246's: 361 violations in 67 families at 60 weeks** — struck
after the day's four relabels (§7.243's seed price, §7.245's fiscal appropriation, §7.246's two
service-cost fixes; the instrument commit after it shifts the SHOCKS=1 stream by one draw, so
the next run differs again slightly and says so). §7.244 (628/94) remains the record of the
pre-fix world; compare nothing to anything before it.
...
| **UNEMPLOYMENT RATCHETS ~0.5pp/WEEK IN ALL FOUR REGIONS, AND ITS OLD ATTRIBUTION IS VOID** | §7.246: with the price level sane (CPI ×2.71/60wk), goods fill at 0.453, the supplier-share family at zero and the capex famine INVERTED (bids 0.56x capacity built), unemployment still climbs monotonically from ~week 10 to 39.7/30.6/39.7/35.9 at week 60. **§7.179's attribution — the capital-goods famine as the accumulated cost behind ~29% unemployment — no longer stands.** This is now the model's top open mechanism. Next action: a fresh diagnosis at the departure (~w10), per §7.224's method — who sheds, through which branch (the affordability rule, the real-growth signal, quits), and whether the §7.245-noted aggregate-vs-own-price deflator mismatch in `desiredEmploymentGrowthAnnual` (region CPI deflates every firm's nominal growth; the level path uses the firm's OWN price) is the residual wrong coupling. Owner: LAB/IND. |
...
| **THE BOOKS PRINT THEIR DAMPERS** | The engine states its own failure condition: a name clamped for weeks means the posted schedules disagree with the printed level and **the print is the damper, not the market.** §7.246 (the run to judge against): **1,990 persistently bound, worst streak the full 60 weeks** (§7.244: 2,189; the old 947 benchmark is void). §7.197 records the level as the honest shape of deleting the residual dealer: **the pressure was always there.** **Do not widen the damper.** The well-posed question is: who buys when the holders as a group want less than they hold? Owners: SCALE (the float half), and the measurement run. |
