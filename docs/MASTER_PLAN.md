...
| 2 | foundation | **SUPPLY/CHAIN — the economy cannot make what it is asked for** *(promoted from item 20, 2026-08-30)* | **THE UNBLOCKING ROW (§7.179).** Two consecutive CORRECT changes could not land because each removed a cushion and pushed unemployment over its band (§7.177, §7.179) — the economy runs at ~29% and the band has become the binding constraint on what can be built. **That 29% is not a labour defect** (§7.149 fixed the one that existed); it is the accumulated cost of the SUPPLY side, and this row owns it: **the capital-goods sector fills ~8% of the capex bid at it (§7.168), and its capacity was seeded off a frozen number (§7.178's remaining half — a genuine fixed point, since firms are generated FROM category demand)**; recipes are one tier deep against real chains that are graphs; and CAP's capacity-off-PP&E (§7.177) restores the moment this closes. CHAIN-D and CHAIN-E are done (§7.117-118, §7.120). Prereqs BP1 and IND10/11, both now done. |
