...
**THE RUN TO JUDGE AGAINST IS §7.247's: 351 violations in 60 families at 60 weeks** (the
demand-pull + cap + repo-fix tree). The day's earlier reference runs — §7.244 (628/94, the
post-§7.242 baseline), §7.246 (361/67, the service-cost tree) — remain the records of their
own worlds; compare nothing across them without naming the relabel between.
...
| **THE WEEK-52+ SEAM: teleporting unemployment, a 10Y step, and a terminal-week price spike** | The monotone ratchet is CLOSED by §7.247's demand-pull hiring (path 13–24% through w59 against the climb to 40%), and what remains is localized: **single-week unemployment teleports** (USA 24→9→27% inside two weeks, EUR likewise — almost certainly the §7.244 vacancy overflow's family: an unguarded ratio in the labour series), **the 10Y stepping 4.69→7.30 at w52 and holding**, and **ONE terminal week (w60: pi 2110, GDP +19% in a week) carrying the run's whole CPI ×39.5.** Also standing from §7.247: full employment re-heats the goods market — fill 0.296 against §7.246's 0.453, supplier shares failing in NEW categories (EUR passenger_vehicles, JPN semiconductors) — employment and the price level traded places, and BOTH mechanisms stay (§1.20). Next action: measure w52 and w60 directly — what steps the 10Y (a calendar? an auction?), what fires in the terminal week, and the week the vacancy series first departs sanity. Also the §7.245-noted deflator mismatches in `desiredEmploymentGrowthAnnual` (aggregate CPI vs own price; 52-week inflation vs 12-week annualized growth) remain unfixed and now READ against a sane price level. Owner: LAB/IND. |
