...
2. **The CDS PFE add-on by rating** (CEM's 5%/10% IG/sub-IG split) — one profile fact becomes a
   function of the reference's rating when a measurement asks for it.
...
| **ONE QUANTITY, MANY AUTHORS — remainder** | §7.338 closed the seller PD (structural model), book depreciation (the programme's rule) and the labour force (`laborForceCount`). Still open: two consumer loss curves in one function (household credit). |
| **THE SEED'S LEVEL** | **CLOSED AT THE OWNER (§7.338):** the wedge was the seed price rule; coverage is fitted at sub-unit grain; the shutdown test runs on avoidable cost. Remainder: goods fill 0.756 at 13 weeks (was 0.687) — the rest is the contract book's owing units and the labour path; measure on the reference. |
