...
test('the weekly charge is the net plant at that rate over the year', () => {
  ...
  const plant = [{ costLocal: 5200, enteredServiceWeek: 10, usefulLifeYears: 10, kind: 'heavy_equipment' }];
  assert.ok(Math.abs(weeklyCapitalChargeLocal({ beta: 1 }, plant, 0.04, 10) - (5200 * (0.04 + EQUITY_RISK_PREMIUM)) / 52) < 1e-9);
  assert.equal(weeklyCapitalChargeLocal({ beta: 1 }, [{ costLocal: 100, enteredServiceWeek: 0, usefulLifeYears: 1, kind: 'heavy_equipment' }], 0.04, 52), 0,
