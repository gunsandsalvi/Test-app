...
const identityResidual = (s: BankingSector, cashUSD = CASH, lines = linesOf(), facilityBookUSD = FAC) =>
  bankAssumedLiabilitiesUSD(s, lines) + (s.centralBankLoanUSD ?? 0) + s.bankEquityUSD - bankSheetAssetsUSD(s, cashUSD, facilityBookUSD);

test('PCA: closed below the ratio, open above it, closed at negative capital with no RWA', () => {
  assert.equal(isBankUnderPca(sheet({ bankEquityUSD: 100 * PCA_CAPITAL_RATIO - 1 }), FAC), true);
  assert.equal(isBankUnderPca(sheet({ bankEquityUSD: 100 * PCA_CAPITAL_RATIO + 1 }), FAC), false);
  assert.equal(isBankUnderPca(sheet({ bankEquityUSD: -1 }), 0), true);
});
...
test('the assuming bank is the best-capitalised peer above the floor, by equity; never one under PCA', () => {
  const cands = [
    { id: 'weak', sheet: sheet({ bankEquityUSD: 1 }), facilityBookUSD: FAC },
    { id: 'big', sheet: sheet({ bankEquityUSD: 12 }), facilityBookUSD: FAC },
    { id: 'bigger-but-thin', sheet: sheet({ bankEquityUSD: 30 }), facilityBookUSD: 1000 },
  ];
