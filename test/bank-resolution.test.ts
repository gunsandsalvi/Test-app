...
const sheet = (over: Partial<BankingSector> = {}): BankingSector => ({
  ...
  moneySupplyM2USD: 80, itemizedHoldings: [], srfBorrowingLocal: 0, onRrpLendingLocal: 0,
  corpBondDealerInventory: [], sovereignBondHoldingsByBond: { t10: 20 }, sovBondDealerInventory: [],
  loanDealerInventory: [], repoLentLocal: 0, repoBorrowedLocal: 0, repoEncumberedCollateralUSD: 0,
...
const identityResidual = (s: BankingSector, cashLocal = CASH, lines = linesOf(), facilityBookLocal = FAC) =>
  bankAssumedLiabilitiesUSD(s, lines) + (s.centralBankLoanLocal ?? 0) + s.bankEquityLocal - bankSheetAssetsUSD(s, cashLocal, facilityBookLocal);
...
test('the transfer closes both sheets: acquirer takes every line, target keeps only cash and the matching equity', () => {
  const F = sheet({
    householdLoans: [{ kind: 'MORTGAGE', principalLocal: 40, vintages: [{ principalLocal: 40, originationCollateralUSD: 60, originationHomePriceUSD: 300000, rateAnnual: 0.05, wamWeeks: 900, fixedForWeeks: 100, originatedWeek: 0 }], wacAnnual: 0.05 }],
  ...
  const A = sheet({
    householdLoans: [{ kind: 'MORTGAGE', principalLocal: 10, vintages: [{ principalLocal: 10, originationCollateralUSD: 15, originationHomePriceUSD: 250000, rateAnnual: 0.04, wamWeeks: 800, fixedForWeeks: 50, originatedWeek: 0 }], wacAnnual: 0.04 }],
  ...
  A.bankEquityLocal += plan.netBookUSD - cash;
  ...
  aCash += plan.guaranteeUSD - plan.estateUSD; A.bankEquityLocal += plan.guaranteeUSD - plan.estateUSD;
  assert.ok(Math.abs(identityResidual(A, aCash, aLines, aFac)) < 1e-9);
  assert.ok(Math.abs(A.bankEquityLocal - (6 + plan.acquirerCapitalUSD)) < 1e-9, 'the acquirer gains exactly the capital the book needs');
  assert.equal(bankSheetAssetsUSD(F, fCash, fFac), 0);
  assert.equal(bankAssumedLiabilitiesUSD(F, fLeft) + F.bankEquityLocal, 0);
