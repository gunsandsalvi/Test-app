...
const lot = (unitsHeld: number, unitPriceUSD: number, acquiredWeek: number): CostedLot =>
  ({ unitsHeld, unitPriceUSD, acquiredWeek });

test('the oldest lot goes first, whatever order it is stored in', () => {
  const { costUSD, unitsTaken } = consumeLotsFifo([lot(10, 5, 9), lot(10, 1, 2)], 10);
  assert.equal(unitsTaken, 10);
  assert.equal(costUSD, 10, 'the week-2 lot at $1 is consumed before the week-9 lot at $5');
});
...
test('cost is what the units COST, not what they are worth now', () => {
  ...
  assert.equal(cheapFirst.costUSD, 5);
});
...
test('a partial lot is split, so no units are created or destroyed', () => {
  ...
  assert.equal(remaining[0].unitPriceUSD, 2, 'the remainder keeps its own cost');
});
...
test('per-lot costs are returned in consumption order, and they sum to the total', () => {
  ...
  assert.equal(r.costsUSD.reduce((a, c) => a + c, 0), r.costUSD);
});
...
test('a warehouse is not free, and the charge is reported separately from the stock', () => {
  const { stock, totalCostUSD } = chargeCarryingCost(
    { grain: { unitsHeld: 100, valueUSD: 5200 } }, () => 0.52);
  assert.equal(totalCostUSD, 52, '52% a year is 1% a week of 5200');
  assert.equal(stock.grain.valueUSD, 5148);
...
test('carrying cost never drives a stock value negative', () => {
  const { stock } = chargeCarryingCost({ x: { unitsHeld: 1, valueUSD: 10 } }, () => 5200);
  assert.ok(stock.x.valueUSD >= 0);
});
