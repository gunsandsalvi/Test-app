...
const firm = (over: Record<string, unknown> = {}): Company => ({
  ...
  growthCapex: 52e6, maintenanceCapex: 52e6,
  annualRevenue: 2e9, stockPrice: 10, eps: 1, ...over,
} as unknown as Company);
const plant = [{ costLocal: 1e9, enteredServiceWeek: 10 - 0.4 * 520, usefulLifeYears: 10, kind: 'heavy_equipment' }];
...
test('cheap debt is raised toward the target at the management\'s pace, into its own programme', () => {
  const d = decideCorporateFinancing({ ...base, comp: firm(), plant, totalDebtLocal: 4e8 });
  ...
  const idle = decideCorporateFinancing({ ...base, comp: firm({ growthCapex: 0, maintenanceCapex: 0 }), plant, totalDebtLocal: 4e8 });
...
test('above its own target a management pays down toward it, over its horizon', () => {
  ...
  const d = decideCorporateFinancing({ ...base, comp: averse, plant, totalDebtLocal: 1.6e9 });
