...
test('cheap debt is raised toward the target at the management\'s pace, into its own programme', () => {
  const d = decideCorporateFinancing({ ...base, comp: firm(), totalDebtLocal: 4e8 });
  ...
  const idle = decideCorporateFinancing({ ...base, comp: firm({ growthCapex: 0, maintenanceCapex: 0 }), totalDebtLocal: 4e8 });
...
test('above its own target a management pays down toward it, over its horizon', () => {
  ...
  const d = decideCorporateFinancing({ ...base, comp: averse, totalDebtLocal: 1.6e9 });
