...
import { plantVintagesOf } from '../src/engine/ledger/plant-ledger';
...
import { overPledgedByBond, PLEDGE_ROUNDING_TOLERANCE_LOCAL } from '../src/domain/collateral';
...
const institutionalBookOf = (s: GameState, region: RegionId) =>
  s.institutionalEntities
    ...
    .reduce(
      (sum, e) =>
        sum + entityCashOf(ensureV2(s), e) + ((e as { repoLentLocal?: number }).repoLentLocal ?? 0) + ((e as { rrpLentLocal?: number }).rrpLentLocal ?? 0)
    ...
    state.institutionalEntities
      ...
      .forEach((mmf) => {
        const bookLocal = entityCashOf(ensureV2(state), mmf) + ((mmf as { repoLentLocal?: number }).repoLentLocal ?? 0)
          + ((mmf as { rrpLentLocal?: number }).rrpLentLocal ?? 0)
          + mmf.itemizedHoldings.reduce((x, h) => x + h.quantityOrNotionalLocal, 0);
        const sharesLocal = mmf.mmfSharesOutstandingLocal ?? 0;
        if (sharesLocal > 1e9 && Math.abs(bookLocal - sharesLocal) / sharesLocal > 0.02) {
          violations.push({
            week,
            message: `${region} money fund book ${(bookLocal / 1e9).toFixed(1)}B departs its $1-NAV share liability ${(sharesLocal / 1e9).toFixed(1)}B by more than 2% — a subscription or redemption moved only one side`,
          });
        }
      });
  });
 ...
 *
 *  - `unresolvedLocal` is money that found no account: a party that does not exist, or a holder
 *    with no bank. Non-zero means dollars left the system, which is the §7.86 defect's shape.
 *  - `clearingHouseResidualLocal` is what the cleared books' central counterparty was left
 ...
 *  - `centralBankResidualLocal` is bank reserves plus the treasury account net of what the
...
function checkSettlementClosed(state: GameState, week: number) {
  ...
  if (!s) return;
  if (Math.abs(s.unresolvedLocal) > 1e6) {
    violations.push({
      week,
      message: `settlement left ${((s.unresolvedLocal) / 1e9).toFixed(3)}B unresolved — a payment found no account`,
    });
  }
  if (Math.abs(s.clearingHouseResidualLocal) > 1e6) {
    violations.push({
      week,
      message: `clearing house left holding ${((s.clearingHouseResidualLocal) / 1e9).toFixed(3)}B — a cleared book settled one side only`,
    });
  }
  if (Math.abs(s.centralBankResidualLocal) > 1e6) {
    violations.push({
      week,
      message: `central bank liabilities moved by ${((s.centralBankResidualLocal) / 1e9).toFixed(3)}B outside its own issuance — reserves and the treasury account did not net`,
    });
  }
}
...
function checkGuards(state: GameState, week: number) {
  ...
  state.companies.forEach((c: Company) => {
    ...
    (c.productLines || []).forEach((pl: ProductLine) => {
      ...
      shareBySubUnit.set(key, (shareBySubUnit.get(key) ?? 0) + (pl.categoryMarketShare));
    });
  });
  ...
  REGION_IDS.forEach((regionId) => {
    ...
    // not a dead market. `fundableNeedLocal` is what a borrower could actually fund.
    const needLocal = reg.repoFundableNeedLocal ?? 0;
    const clearedLocal = reg.repoClearedVolumeLocal ?? 0;
    if (needLocal > 0 && !(clearedLocal > 0)) {
      violations.push({
        week,
        message: `${regionId} repo: ${(needLocal / 1e9).toFixed(2)}B of fundable borrowing need and the session cleared ZERO volume — the printed rate is the early-return default, not a market`,
      });
    }
  });

  // 3. A holding CEILING may not equal the position it bounds. `investableSurplusLocal` was the
...
function checkCentralBankIdentity(state: GameState, week: number) {
  REGION_IDS_SEED_ORDER.forEach((region) => {
    ...
    const orderedLocal = cb.lastOrderPlacedLocal;
    const filledLocal = cb.lastOpenMarketPurchasesLocal;
    // §7.246: the `orderedLocal > 0` arm made this VACUOUS in exactly the failure it exists for —
    // a fill against NO order is the purest forced placement, and the guard skipped it.
    if (filledLocal > 0 && filledLocal > orderedLocal * 1.01 + 1e6) {
      violations.push({
        week,
    ...
    const outlays = reg.governmentOutlaysLocal;
    if (outlays !== undefined) {
      const spent = reg.governmentProcurementSpentLocal ?? 0;
      const unspent = reg.unspentProcurementBudgetLocal ?? 0;
      // §5-STRUCT step 3 — ASK THE GOVERNMENT, and this is why the §6.1 row never closed. The
      // check used to read `outlays > governmentSpendingWeeklyLocal * 1.5`: a stated 50% tolerance against
      ...
      if (spent > 0 && overrunLocal > 1e6) {
        violations.push({
          week,
          message: `${region} government outlays ${(outlays / 1e9).toFixed(2)}B exceed its budget ${((outlays - overrunLocal) / 1e9).toFixed(2)}B by ${(overrunLocal / 1e9).toFixed(2)}B — contractual ${(contractualLocal / 1e9).toFixed(2)}B, discretionary ${(discretionaryLocal / 1e9).toFixed(2)}B`,
        });
      }
...
function checkLaborMarketIdentity(state: GameState, week: number) {
  REGION_IDS_SEED_ORDER.forEach((region) => {
    ...
    const poolEmployed = Object.values(reg.occupationPools).reduce((a: number, p: OccupationPool) => a + (p.employed), 0);
    ...
    const laborForce = reg.totalPopulation * (1 - (reg.nonEmployablePct)) * reg.laborForceParticipation;
...
function checkHouseholdCohortIdentity(state: GameState, week: number) {
  REGION_IDS_SEED_ORDER.forEach((region) => {
    ...
    const sumDisposable = cohorts.reduce((a, c) => a + c.disposableIncomeLocal, 0);
    ...
    const sumSavings = cohorts.reduce((a, c) => a + c.savingsLocal, 0);
    ...
    if (agg > 0 && Math.abs(impliedRate - (hs.savingsRate)) > 1e-3) {
      violations.push({
        week,
        message: `${region}: cohort savings imply a rate of ${(impliedRate * 100).toFixed(2)}% but the sector publishes ${(((hs.savingsRate)) * 100).toFixed(2)}% — the savings rate is not the cohorts' own measurement`,
      });
    }
    ...
    if (bankDepositsLocal > 0) {
      ...
      if (Math.abs(hsView - bankDepositsLocal) / bankDepositsLocal > 1e-3) {
        violations.push({
          week,
          message: `${region}: household deposits ${(hsView / 1e9).toFixed(1)}B (incl. in-flight) vs banks' household-deposit lines ${(bankDepositsLocal / 1e9).toFixed(1)}B — the two representations are drifting again`,
        });
      }
    }
    ...
    if ((hs.netWorthLocal) !== 0) {
      const tierSum = Object.values(wd).reduce((a: number, t: { shareOfNetWorthLocal?: number }) => a + (t.shareOfNetWorthLocal ?? 0), 0);
      if (Math.abs(tierSum - (hs.netWorthLocal)) / Math.max(1, Math.abs(hs.netWorthLocal)) > 1e-3) {
        violations.push({
          week,
          message: `${region}: tier net worths sum to ${(tierSum / 1e9).toFixed(1)}B against an aggregate of ${((hs.netWorthLocal) / 1e9).toFixed(1)}B — the tier split is not a partition of the real book`,
        });
      }
    }
    ...
    const sumBudgets = cohorts.reduce((a, c) => a + c.consumptionBudgetLocal, 0);
    ...
    const sumEffectiveDs = cohorts.reduce((a, c) => a + c.debtServiceLocal, 0);
    ...
    const sumConsumptionTax = cohorts.reduce((a, c) => a + (c.consumptionTaxLocal), 0);
    const expectedBudgets = sumDisposable - sumSavings
      - sumEffectiveDs + (hs.capitalReceiptsAnnualLocal ?? 0) - sumConsumptionTax;
...
function checkBeneficiaryClaimsHaveHolders(state: GameState, week: number) {
  ...
  const heldLocal = REGION_IDS_SEED_ORDER
    .reduce((sum, r) => sum + (state.regions[r].householdState.institutionalClaimsLocal), 0);
  if (owedLocal <= 0 && heldLocal <= 0) return;
  const gapLocal = Math.abs(owedLocal - heldLocal);
  if (gapLocal / Math.max(1, owedLocal) > 0.001) {
    violations.push({
      week,
      message:
        `Beneficiary claims do not reconcile: institutions owe ${(owedLocal / 1e9).toFixed(1)}B, ` +
...
function checkNavIdentity(state: GameState, week: number) {
  ...
  const diff = Math.abs(state.portfolio.navLocal - expectedNav);
  if (diff > 0.01) {
    violations.push({
      week,
      message: `NAV identity mismatch: portfolio.navLocal=${state.portfolio.navLocal}, expected=${expectedNav} (cash=${state.portfolio.cashLocal}, unrealized=${totalUnrealizedPnL})`
    });
  }
}
...
function checkMarkToMarketUnfreezesPortfolio(): Violation | null {
  ...
  const postNav = state.portfolio.navLocal;
  ...
  if (postCompany && postCompany.stockPrice !== company.stockPrice && postNav === preNav) {
    return { week: state.currentWeek, message: `Portfolio NAV frozen: navLocal unchanged after weekly advance despite ${company.ticker} price moving ${company.stockPrice} -> ${postCompany.stockPrice} (nav=${preNav})` };
  }
...
function checkUndersubscribedSovereignAuctionRaisesYield(): Violation | null {
  ...
  // (bankEquityLocal / sectorEquityLocal) that the clearing engine stopped reading when sovereign
  ...
  shocked.companies.forEach(c => {
    if (c.region === 'USA' && c.bankBalanceSheet) {
      const beforeLocal = bankReservesOf(ensureV2(shocked), c.id);
      adjustBankReserves(ensureV2(shocked), c.id, beforeLocal * 0.01 - beforeLocal); // the drain: the account keeps 1%
    ...
    report(s, weeks) {
      ...
      REGIONS.forEach(r => {
        ...
        const instLiab = s.institutionalEntities.filter(e => e.region === r && !e.isDefaulted)
          .reduce((a, e) => a + (e.beneficiaryLiabilityLocal ?? 0), 0);
        const held = hs.institutionalClaimsLocal;
        const gap = Math.abs(instLiab - held) / Math.max(1, instLiab);
        const nwParts = householdDepositsOf(ensureV2(s), r) + (hs.mmfSharesLocal ?? 0) + (hs.equityHoldingsLocal)
          + (hs.housingStockLocal)
          - ((hs.mortgageDebtLocal) + (hs.creditCardDebtLocal) + (hs.otherConsumerLoanDebtLocal));
        const nwGap = Math.abs(nwParts - (hs.netWorthLocal)) / Math.max(1, Math.abs(hs.netWorthLocal));
        ...
        const tierGap = Math.abs(tierSum - (hs.netWorthLocal)) / Math.max(1, Math.abs(hs.netWorthLocal));
      ...
      REGIONS.forEach(r => {
        ...
        const hDepositsLocal = householdDepositsOf(ensureV2(s), r);
        const dep = Math.max(0, hDepositsLocal + (h.mmfSharesLocal ?? 0));
        const ds = Math.max(0, h.weeklyDebtServiceLocal ?? 0);
        const cons = Math.max(0, s.regions[r].estimatedHouseholdIncomeLocal) * (1 - (h.savingsRate)) / 52;
        ...
        out.push(`  ${r}: liquid ${B(dep)} vs committed ${B(committed)}/wk = ${(committed > 0 ? dep / committed : 0).toFixed(1)} weeks of cover | savings rate ${pct(h.savingsRate)} | income ${B(s.regions[r].estimatedHouseholdIncomeLocal)}`);
        ...
        const incomeLocal = Math.max(0, s.regions[r].estimatedHouseholdIncomeLocal);
        const floorLocal = (incomeLocal / 52) * 12;
        const headroomLocal = Math.max(0, hDepositsLocal - floorLocal);
        const gapLocal = Math.max(0, -(incomeLocal * (h.savingsRate)) / 52);
        ...
        const sellableLocal = etfRows.reduce((a: number, x) => {
          ...
          const nav = sh > 0 && f ? ((f.itemizedHoldings).reduce((b: number, hh: ItemizedHolding) => b + (hh.quantityOrNotionalLocal), 0) + Math.max(0, entityCashOf(ensureV2(s), f))) / sh : 0;
          return a + (x.shares) * nav;
        }, 0);
        ...
        out.push(`      sellable (fund shares) ${B(sellableLocal)} of ${B(h.equityHoldingsLocal)} total household equity — the rest has no trading channel`);
      });
      out.push('--- the mortgage book as a CROSS-SECTION: E[f(LTV)] against f(E[LTV]) ---');
      REGIONS.forEach(r => {
        const price = Math.max(0, s.regions[r].housingMarket.medianHomePriceLocal);
        ...
        const underwaterLocal = vs.filter(v => ltvOf(v) > 0.75).reduce((a, v) => a + v.principalLocal, 0);
        ...
        out.push(`      E[f(LTV)] ${eOfF.toFixed(4)}  vs  f(E[LTV]) ${fOfE.toFixed(4)}  = ${(eOfF / Math.max(1e-9, fOfE)).toFixed(2)}x  | above the kink: ${pct(book > 0 ? underwaterLocal / book : 0)} of the book`);
        ...
        const oldStyleLtv = (() => {
          const h = s.regions[r].householdState;
          const stock = Math.max(0, h.housingStockLocal);
          return stock > 0 ? Math.max(0, h.mortgageDebtLocal) / stock : 0;
        })();
        ...
        const hh = Math.max(1, (reg2.totalPopulation) / 2.5);
        const wkInc = Math.max(0, reg2.estimatedHouseholdIncomeLocal) / 52 / hh;
        const mortRate = Math.max(0.005, (reg2.zeroRates.tenor10Y) + 0.017);
        ...
        const resettingLocal = vs.filter((v) => v.fixedForWeeks <= 52).reduce((a, v) => a + v.principalLocal, 0);
        const coupons = vs.map((v) => v.rateAnnual).sort((a, b) => a - b);
        out.push(`      coupons p10 ${pct(coupons[Math.floor(coupons.length * 0.1)])} p90 ${pct(coupons[Math.floor(coupons.length * 0.9)])} | resetting within a year: ${pct(book > 0 ? resettingLocal / book : 0)} of the book`);
      ...
      out.push(`  net worth / income (USA): ${((s.regions.USA.householdState.netWorthLocal) / s.regions.USA.estimatedHouseholdIncomeLocal).toFixed(2)}x`);
      return out;
    },
    shock(snapshot, rngState, shockWeek, weeks) {
      ...
      const run = (st: GameState, kill: boolean) => {
        ...
        for (let i = 0; i < horizon; i++) {
          ...
          o.c.push((reg.householdState.cohorts ?? []).reduce((a, ch) => a + ch.consumptionBudgetLocal, 0));
...
function couponReceipts(s: GameState, region: RegionId) {
  ...
  const insts = s.institutionalEntities
    ...
    .reduce((a: number, e: InstitutionalEntity) => a + e.itemizedHoldings
      ...
      .reduce((x: number, h: ItemizedHolding) => x + ((h.quantityOrNotionalLocal) * rate(h.instrumentId)) / 52, 0), 0);
...
const pubModule: HarnessModule = (() => {
  ...
  return {
    ...
    week(_prev, s) {
      ...
      const dec = decomposeGovernmentSpending(reg.governmentSpendingWeeklyLocal, reg.governmentInterestWeeklyLocal ?? 0,
        GOV_PROCUREMENT_SHARE_OF_SPENDING, reg.fiscalStanceScore);
      series.interestShare.push(dec.interestLocal / Math.max(1, reg.governmentSpendingWeeklyLocal));
      series.procShare.push(dec.procurementBudgetLocal / Math.max(1, reg.governmentSpendingWeeklyLocal));
      series.procSpent.push(reg.governmentProcurementSpentLocal ?? 0);
      ...
      series.debtGdp.push(reg.debtToGdpPctBottomUp);
    ...
    shock(snapshot, rngState, shockWeek, weeks) {
      ...
      const run = (st: GameState, shocked: boolean) => {
        ...
        let x = clone(st);
        if (shocked) {
          materializeGovLadder(ensureV2(x), 'USA').forEach((t: GovDebtTranche) => { t.couponRate = (t.couponRate) * 4 + 0.04; });
        }
        ...
        for (let i = 0; i < horizon; i++) {
          ...
          const dec = decomposeGovernmentSpending(reg.governmentSpendingWeeklyLocal, reg.governmentInterestWeeklyLocal ?? 0,
            GOV_PROCUREMENT_SHARE_OF_SPENDING, reg.fiscalStanceScore);
          o.interest.push(dec.interestLocal); o.transfers.push(dec.transfersLocal);
          o.proc.push(reg.governmentProcurementSpentLocal ?? 0);
          o.debt.push(reg.debtToGdpPctBottomUp);
        }
        return o;
      };
      const ctl = run(snapshot, false);
...
const xbModule: HarnessModule = (() => {
  ...
  return {
    ...
    report(s) {
      ...
      out.push(`  logistics revenue ${B(carriers.reduce((a, c) => a + c.annualRevenue, 0))} = ${pct(carriers.reduce((a, c) => a + c.annualRevenue, 0) / Math.max(1, REGIONS.reduce((a, r) => a + (s.regions[r].derivedNominalGdpLocal), 0)))} of world GDP [real: 5-6%]`);
      ...
      const withRevenue = carriers.filter(c => (c.carrierFleet?.lastWeekFreightRevenueLocal ?? 0) > 0);
      ...
      const subs = s.companies.filter((c) => c.parentId && isActiveCompany(c));
      out.push(`  subsidiaries ${subs.length}${subs.length > 0
        ? ` (${subs.slice(0, 6).map((c) => `${c.parentId}->${c.ticker}@${c.region}`).join(' ')})` : ''}`);
      ...
      tradeInvoicesOf(ensureV2(s)).forEach((inv) => {
        ...
        crossLocal += usd;
        if (groupOf.get(inv.sellerId) !== undefined
          && groupOf.get(inv.sellerId) === groupOf.get(inv.buyerId)) intraLocal += usd;
      });
      out.push(`  intra-firm share of cross-border invoices: ${crossLocal > 0 ? pct(intraLocal / crossLocal) : 'n/a (no cross-border book)'} [real: ~one third; EMERGES from who owns whom]`);
  ...
  {
    const overdraftLocal = Number(s.lastCashOverdraftLocal ?? 0);
    if (overdraftLocal > 0) out.push(`  institutions overdrawn (money spent that was not there): ${B(overdraftLocal)}`);
  }
      ...
      REGIONS.forEach(r => {
        ...
        out.push(`  ${r.padEnd(4)} fx ${fx.toFixed(4)}  mean converted price vs USA ${(mean.sum / Math.max(1, mean.n)).toFixed(3)} [1.000 = law of one price]  fxReserves ${B(cb ? centralBankFxReservesLocal(cb) : 0)}`);
      });
...
const indModule: HarnessModule = (() => {
  ...
  const allContracts = (s: GameState) => {
    ...
    const out = new Map<string, {
      ...
      weeksRemaining: number; escalationBaseLocal: number; prepaidLocal: number;
    }>();
    REGIONS.forEach(r => forEachContract(v2, r, (row, supplierKey, customerKey, subUnitId) => {
      out.set(`${supplierKey}|${customerKey}|${subUnitId}`, {
        ...
        weeksRemaining: CT.weeksRemaining[row], escalationBaseLocal: CT.escalationBaseLocal[row],
        prepaidLocal: CT.prepaidLocal[row],
      });
    }));
    return out;
  };
  ...
  report(s) {
    ...
    const totalWipLocal = s.companies.reduce((a, c) => {
    ...
    out.push(`  work in progress carried across every firm: ${B(totalWipLocal)}`);
    ...
    {
      const wi = s.companies.filter(c => isActiveCompany(c) && (c.employeeCount) > 0)
      ...
      if (wi.length > 2) {
        ...
        const tgt = s.companies.filter(c => isActiveCompany(c) && (c.employeeCount) > 0 && c.annualRevenue > 0)
      ...
      REGIONS.slice(0, 1).forEach(r => {
        ...
        (['BOTTOM_50', 'NEXT_40', 'TOP_9', 'TOP_1'] as const).forEach(t => {
          ...
          const wage = rows.reduce((a, c) => a + (c.wageIncomeLocal), 0);
          const cap = rows.reduce((a, c) => a + (c.capitalIncomeLocal), 0);
          const tr = rows.reduce((a, c) => a + (c.transferIncomeLocal) + (c.unemploymentBenefitsLocal), 0);
    ...
    // is too coarse and the number is secretly doing work.
    {
      ...
      if (vs2.length >= 4) {
        const price = Math.max(1, s.regions.USA.housingMarket.medianHomePriceLocal);
    ...
    REGIONS.forEach(r => {
      ...
      const netPpe = firms.reduce((a, c) => a + plantNetLocal(plantVintagesOf(ensureV2(s), c.id), s.currentWeek), 0);
      const coc = Math.max(0, (s.regions[r].zeroRates.tenor10Y));
      const below = firms.filter(c => {
        const np = plantNetLocal(plantVintagesOf(ensureV2(s), c.id), s.currentWeek);
        return c.ebitda < np * (coc + (c.beta) * 0.05);
      }).length;
      ...
      const dep = firms.reduce((a, c) => a + plantDepreciationAnnualLocal(plantVintagesOf(ensureV2(s), c.id), s.currentWeek), 0);
      const capexA = firms.reduce((a, c) => a + (c.capex), 0);
      ...
      if (r === 'USA') {
        ...
        capexCats.forEach(su => {
          ...
          out.push(`      ${su.padEnd(24)} supplied/demanded ${(d > 0 ? sup / d : 0).toFixed(2)}x  (px ${(cd.unitPriceLocal ?? 0).toFixed(0)} vs base ${(cd.baseUnitPriceLocal ?? 0).toFixed(0)})`);
        });
      }
    ...
    let prepaidTotalLocal = 0;
    [...allContracts(s).values()].forEach(c => {
      const v = c.prepaidLocal;
      if (!(v > 0)) return;
      prepaidTotalLocal += v;
      prepaidBySupplier.set(c.supplierCompanyId, (prepaidBySupplier.get(c.supplierCompanyId) ?? 0) + v);
    });
    out.push(`  customer deposits held: ${B(prepaidTotalLocal)} across ${prepaidBySupplier.size} suppliers`);
    ...
    [{ lo: 0, hi: 0.5, label: 'lead 0wk   ' }, { lo: 0.5, hi: 6, label: 'lead 1-5wk ' }, { lo: 6, hi: 1e9, label: 'lead 6+wk  ' }].forEach(b => {
      const firms = s.companies.filter(c => {
        ...
        const w = lines.reduce((a: number, l: ProductLine) => a + (l.revenueShare), 0);
        const lead = w > 0 ? lines.reduce((a: number, l: ProductLine) => a + (l.revenueShare) * productionLeadWeeksOf(l.subUnitId), 0) / w : 0;
    ...
    out.push('--- §5-TAXR: the tax base against the book ---');
    {
      ...
      const carryFirms = firms.filter(c => (c.taxLossCarryforwardLocal ?? 0) > 0);
      ...
      const netBookLocal = firms.reduce((a, c) => a + plantNetLocal(plantVintagesOf(ensureV2(s), c.id), s.currentWeek), 0);
    ...
    out.push('--- DRV: the one derivative book (§5-DRV) ---');
    {
      ...
      let pfeLocal = 0;
      for (const c of book) {
        const row = byClass.get(c.classId) ?? { n: 0, notional: 0, settledMarkLocal: 0, banks: 0, firms: 0, institutions: 0 };
        row.n++; row.notional += c.notional; row.settledMarkLocal += c.settledMarkLocal ?? 0;
        for (const p of [c.a, c.b]) {
          if (p.kind === 'BANK') row.banks++; else if (p.kind === 'COMPANY') row.firms++; else row.institutions++;
          if (p.kind === 'BANK') pfeLocal += c.notional * DERIVATIVE_CLASSES[c.classId].pfeAddOnRate;
        }
        byClass.set(c.classId, row);
      }
      out.push(`  ${book.length} live contracts across ${byClass.size} classes at week ${wk}; PFE charged to bank desks ${B(pfeLocal)} (one budget, every class)`);
      byClass.forEach((r, id) => {
        ...
        out.push(`  ${id.padEnd(16)} n=${String(r.n).padStart(5)} (${live} live) notional ${B(r.notional)} | sides: banks ${r.banks} firms ${r.firms} institutions ${r.institutions}`
          + (r.settledMarkLocal !== 0 ? ` | mark settled to A ${B(r.settledMarkLocal)}` : ''));
      });
    }
    ...
    const grossPpeLocal = s.companies.filter(isActiveCompany).reduce((a, c) => a + plantGrossLocal(plantVintagesOf(ensureV2(s), c.id), s.currentWeek), 0);
    ...
    invoices.forEach(iv => {
      ...
      receivableById.set(iv.sellerId, (receivableById.get(iv.sellerId) ?? 0) + usd);
      payableById.set(iv.buyerId, (payableById.get(iv.buyerId) ?? 0) + usd);
    });
    const totalLocal = domesticLocal + crossLocal;
    const receivables = [...receivableById.values()].reduce((a, x) => a + x, 0);
    const payables = [...payableById.values()].reduce((a, x) => a + x, 0);
    out.push(`  ${invoices.length} invoices outstanding, ${B(totalLocal)} — domestic ${B(domesticLocal)} (${pct(totalLocal ? domesticLocal / totalLocal : 0)}), cross-border ${B(crossLocal)}`);
    ...
    const leadOf = (c: Company) => {
      const lines = c.productLines ?? [];
      const w = lines.reduce((a: number, l: ProductLine) => a + (l.revenueShare), 0);
      return w > 0 ? lines.reduce((a: number, l: ProductLine) => a + (l.revenueShare) * productionLeadWeeksOf(l.subUnitId), 0) / w : 0;
    };
    ...
    buckets.forEach(b => {
      ...
      if (firms.length === 0) return;
      const rev = firms.reduce((a, c) => a + c.annualRevenue, 0) / 52;
      const rec = firms.reduce((a, c) => a + (receivableById.get(c.id) ?? 0), 0);
    ...
    const owed = contracts.filter(c => (c.backlogUnits) > 0.0001);
    const weeklyObligation = contracts.reduce((a, c) => a + (c.quantityUnitsPerWeek), 0);
    const backlogUnits = contracts.reduce((a, c) => a + (c.backlogUnits), 0);
    const backlogLocal = contracts.reduce((a, c) => a + (c.backlogUnits) * (c.priceLocal), 0);
    const indexed = contracts.filter(c => (c.escalationBaseLocal) > 0).length;
    const short = contracts.filter(c => (c.shortWeeks) > 0);
    ...
    out.push(`  backlog outstanding ${backlogUnits.toFixed(0)} units / ${B(backlogLocal)}`);
    ...
    out.push(`  under-delivering: ${short.length} contracts on the non-performance clock, worst streak ${short.reduce((a, c) => Math.max(a, c.shortWeeks), 0)} weeks`);
  ...
  week(_prev, state, w) {
    ...
    if (process.env.REGISTER_DUMP && w === (Number(process.env.STATE_DUMP_WEEK) || 1)) {
      ...
      (state.institutionalEntities).forEach((e) => {
        ...
        e.itemizedHoldings.forEach((h) => {
          ...
          cur[0] += h.quantityOrNotionalLocal; cur[1] += h.quantityShares ?? 0;
          agg.set(k, cur);
        });
    ...
    if (process.env.STATE_DUMP && w === (Number(process.env.STATE_DUMP_WEEK) || 1)) {
      ...
      writeFileSync(process.env.STATE_DUMP,
        JSON.stringify(state.companies.map((c) => ({ ...c, cash: cashOf(dumpV2, c), ...(c.bankBalanceSheet ? { reserves: bankReservesOf(dumpV2, c.id), lines: stateDepositLines(state, c) } : {}) })),
  ...
  week(prev, state, w) {
    ...
    REGION_IDS_SEED_ORDER.forEach((region) => {
      ...
      state.institutionalEntities.forEach((e) => {
        ...
        e.itemizedHoldings.forEach((h) => {
          ...
          if (h.instrumentType === 'CORP_BOND' || h.instrumentType === 'LEVERAGED_LOAN' || h.instrumentType === 'COMMERCIAL_PAPER') {
            const usd = h.quantityOrNotionalLocal;
            creditHeldLocal += usd;
            // instrumentId IS the issuer's company.id for the credit classes (domain/banking.ts).
            if (issuerAliveById.get(h.instrumentId) === false) heldOnDeadIssuerLocal += usd;
          }
        });
      });
      ...
      console.log(`  [book] w${w} ${region} ${(total(before) / 1e9).toFixed(1)}B -> ${(total(after) / 1e9).toFixed(1)}B`
        ...
        + ` (on dead issuers ${(heldOnDeadIssuerLocal / 1e9).toFixed(1)}B)`
        + (deltas ? ` | ${deltas}` : ''));
    });
  },
};
  ...
  week(_prev, state, w) {
    if (SPIRAL_PRICES) {
      ...
      if (focus) {
        REGION_IDS_SEED_ORDER.forEach((region) => {
          ...
          const staffed = sup.reduce((a, c) => a + ((c.baselineEmployeeCount) > 0
            ? Math.min(1, (c.employeeCount) / c.baselineEmployeeCount!) : 1), 0);
    ...
    if (process.env.CARRIER === '1') {
      ...
      const line = carriers.slice(0, 12).map((c) => {
        ...
        return `${c.ticker}${flag} cash${(cashOf(ensureV2(state), c) / 1e6).toFixed(0)}M rev${((c.annualRevenue) / 1e6).toFixed(0)}M ni${((c.netIncome) / 1e6).toFixed(1)}M`;
      }).join(' | ');
        ...
        banksOf(state.companies, region)
          .forEach((c) => {
            ...
            console.log(`  [cap] w${w} ${region}:${c.ticker}`
              ...
              + ` cash ${(bankReservesOf(ensureV2(state), c.id) / 1e9).toFixed(2)}B cbloan ${((bs.centralBankLoanLocal ?? 0) / 1e9).toFixed(2)}B`
    ...
    REGION_IDS_SEED_ORDER.forEach((region) => {
      const r = state.regions[region];
      const smeEmployment = r.smePools.reduce((a, s) => a + (s.employment), 0);
      ...
      console.log(`  [spiral] w${w} ${region}`
        ...
        + ` transfers ${((r.governmentTransfersWeeklyLocal ?? 0) / 1e9).toFixed(2)}B`
        + ` payrollGov ${((r.governmentPayrollWeeklyLocal ?? 0) / 1e9).toFixed(2)}B`
        ...
        + ` govEmp ${((r.governmentEmployment) / 1e6).toFixed(2)}M`
...
function weekLine(s: GameState, w: number, newViol: number, totalViol: number, ms: number): string {
  const r = s.regions;
  const u = (id: RegionId) => ((r[id].unemploymentRate) * 100).toFixed(1);
...
function runHarness() {
  ...
  for (let w = 1; w <= WEEKS; w++) {
    ...
    // issuance from `region.nominalGdpLocal` and `region.governmentDeficitPct`, **neither of which
    ...
    // a repair. The real deficit now has an owner (`Government.deficitWeeklyLocal`, §5-STRUCT step 3);
    ...
    banksOf(state.companies).forEach((c: Company) => {
      ...
      const residualLocal: number =
        ...
        lines.householdLocal + lines.corporateLocal + lines.institutionalLocal + lines.ccpLocal + lines.smeLocal + (bs.centralBankLoanLocal ?? 0) + bs.bankEquityLocal + (bs.srfBorrowingLocal) + (bs.repoBorrowedLocal)
      ...
      if (Math.abs(residualLocal) > 5e6 || idTraced) {
        ...
        console.log(`  [bank-identity] w${w} ${c.ticker} resid ${(residualLocal / 1e9).toFixed(2)}B: hhDep ${gb(lines.householdLocal)}B`
          ...
          + ` sov ${gb(sovLocal)}B cash ${gb(reservesLocal)}B`
          + ` repoL ${gb(bs.repoLentLocal)}B rrp ${gb(bs.onRrpLendingLocal)}B`);
      }
      if (Math.abs(residualLocal) > 5e6) {
        violations.push({
          week: w,
          message: `Bank ${c.ticker} balance-sheet identity broken by ${(residualLocal / 1e6).toFixed(1)}M — a flow is missing a leg`
        });
      }
      ...
      if (reservesLocal < -1e6) {
        violations.push({
          week: w,
          message: `Bank ${c.ticker} overdrawn at the central bank by ${(-reservesLocal / 1e6).toFixed(1)}M — a shortfall nothing funded`
        });
      }
    });
      ...
      if (bankHeldLocal - (reg.sovereignCouponPayableLocal ?? 0) > 5e6) {
        violations.push({
          ...
          message: `${regionId} sovereign receivables exceed the treasury's payable by `
            + `${((bankHeldLocal - (reg.sovereignCouponPayableLocal ?? 0)) / 1e6).toFixed(1)}M — `
    ...
    state.institutionalEntities.forEach((e: InstitutionalEntity) => {
      ...
      if (cashLocal < -1e6) {
        violations.push({
          week: w,
          message: `${e.ticker} is overdrawn by ${(-cashLocal / 1e9).toFixed(2)}B — a fund spending money it does not have [id ${e.id} ${e.entityType} ${e.region}]`,
        });
      }
    });
    ...
    REGION_IDS.forEach(regionId => {
      ...
      book.forEach((c) => {
        borrowedBy.set(c.borrowerId, (borrowedBy.get(c.borrowerId) ?? 0) + c.principalLocal);
        const byBond = pledgedBy.get(c.borrowerId) ?? new Map<string, number>();
        (c.collateral).forEach((p) => byBond.set(p.bondId, (byBond.get(p.bondId) ?? 0) + p.faceLocal));
        pledgedBy.set(c.borrowerId, byBond);
      ...
      banksOf(state.companies, regionId).forEach((c: Company) => {
        const bs = c.bankBalanceSheet!;
        const derivedLocal = borrowedBy.get(c.id) ?? 0;
        const sheetLocal = (bs.repoBorrowedLocal) + (bs.srfBorrowingLocal);
        if (Math.abs(derivedLocal - sheetLocal) > 5e6) {
          violations.push({ week: w, message: `Bank ${c.ticker} secured borrowing ${(sheetLocal / 1e9).toFixed(2)}B disagrees with its ${(derivedLocal / 1e9).toFixed(2)}B of repo contracts` });
        }
  ...
  const acquiredBaselineByAcquirer = new Map<EntityId, number>();
  state.companies.forEach(c => {
    if (c.mergerAcquired && c.acquiredById) {
      const targetInit = initialRevenueByTicker.get(c.ticker) ?? 0;
      acquiredBaselineByAcquirer.set(
        c.acquiredById,
        (acquiredBaselineByAcquirer.get(c.acquiredById) ?? 0) + targetInit
      );
    }
  });
  state.companies.forEach(c => {
    const initRev = (initialRevenueByTicker.get(c.ticker) ?? 0) + (acquiredBaselineByAcquirer.get(c.id) ?? 0);
